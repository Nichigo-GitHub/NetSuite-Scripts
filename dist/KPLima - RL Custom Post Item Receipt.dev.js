  ...
  function doPost(requestBody) {
    ...
    try {
      if (utility.isValueValid(requestBody)) {
        ...
        reasonForAdjustment = requestParams.reasonForAdjustment;
        ...
        if (utility.isValueValid(warehouseLocationId) && utility.isValueValid(transactionInternalId) && utility.isValueValid(transactionName) && utility.isValueValid(transactionType)) {
              ...
              if (poStatus == 'pendingReceipt' || poStatus == 'partiallyReceived' || poStatus == 'pendingRefundPartReceived' || poStatus == 'pendingBillPartReceived' || poStatus == 'pendingReceiptPartFulfilled' || poStatus == 'Pending Refund/Partially Received') {
                ...
                if (transactionType == "transferorder" && useitemcostflag == true) {
                  ...
                  if (opentaskSearchResults.length > 50) {
                    ...
                    utility.updateScheduleScriptStatus('post item receipt Mapreduce', currentUserId, 'Submitted', transactionInternalId, transactionType);
                  }

                  if (opentaskSearchResults.length > 0 && isMapReduceScriptInvoked == 'F') {
                    itemReceiptId = inboundLib.consolidatePostItemReceiptforTO(transactionInternalId, warehouseLocationId, opentaskSearchResults);
                  ...
                  if (opentaskSearchResults.length > 0 && isMapReduceScriptInvoked == 'F') {
                    ...
                    if (transactionType == 'returnauthorization') recordType = record.Type.RETURN_AUTHORIZATION;else if (transactionType == 'transferorder') recordType = record.Type.TRANSFER_ORDER;
                    var trecord = null;
                      ...
                      if (prossedLinesArr.indexOf(linenum) == -1) {
                        ...
                        if (transactionType == 'returnauthorization' && (isKitComponent === true || utility.isValueValid(kitDetailsforRMA[itemId]))) {
                          if (utility.isValueValid(kitIRPostObj[parentItem]) && kitIRPostObj[parentItem] > 0) {
                            inboundLib.updateItemReceiptDetailsForRMAKit(trecord, opentaskSearchResults[otItr], opentaskSearchResults, warehouseLocationId, transactionInternalId, kitDetailsforRMA, kitIRPostObj, irUpdateOpenTaskId, splitOpenTaskId);
                            RMAkitIterator++;
                          }
                        } else {
                          inboundLib.consolidatePostItemReceipt(trecord, actQuantity, linenum, itemId, transactionType, batchNo, expiryDate, warehouseLocationId, enterBin, serialArray, opentaskSearchResults, transactionInternalId, otItr);
                          itemIterator++;
                        }
                      }
                    }
                    ...
                    if (trecord != null && trecord != '') {
                      ...
                      trecord.setValue('custbody91', mrrNumber);
                      ...
                      trecord.setValue('custbody1', empSearchResultsvalues[0].id);
                      ...
                      trecord.setValue('custbody2', empSearchResultsvalues[0].id);
                      trecord.setValue('trandate', new Date(Date.parse(date.value)));
                      ...
                      trecord.setValue('custbody239', reasonForAdjustment);
