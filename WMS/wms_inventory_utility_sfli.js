...
define(['N/search', 'N/runtime', 'N/record', 'N/query', 'N/format', './big', './wms_utility', './wms_translator', 'N/task', 'N/config'],
	function (search, runtime, record, query, format, Big, utility, translator, task, config) {
		...
		function _getItemBinInvtDetails(getItemInternalId, strLocation, inventoryStatusFeature,
		...
		function getInvtBinFilters(invtBinFilters, getItemInternalId, strLocation,
			systemRule_AllowExpiredItems, itemType, processType) {
		...
		function _getInvtBinDetails(objBinInvtDetails, vBinLocArr, selectedConversionRate,
			currentConversionRate, vmakeInvAvailFlag, vBinIntIdExcludeArr) {
			var vValidBinId = '';
			var vValidBin = '';
			var vBinQtyAvail = 0;
			...
			for (var invtBinItr in objBinInvtDetails) {
				...
				vBinStatus = objBinInvtDetails[invtBinItr]['statusText'];
				//vZone = objBinInvtDetails[invtBinItr]['custrecord_wmsse_zoneText'];
				...
				if ((parseFloat(vBinQtyAvail) > 0) && (vBinIntIdExcludeArr.indexOf(vValidBinId) == -1)) {
					if (utility.isValueValid(selectedConversionRate) && utility.isValueValid(currentConversionRate) &&
						(selectedConversionRate != currentConversionRate)) {
		...
		function inventoryInvTransfer(invtransferObj) {
			...
			var statusArray = invtransferObj.statusArray;
			...
			if (itemType == "inventoryitem" || itemType == "assemblyitem") {
				...
				columns.push(search.createColumn('custrecord_wmsse_ser_no'));
				columns.push(search.createColumn('name'));
				columns.push(search.createColumn('custrecord_serial_inventorystatus'));
				var SrchRecordTmpSeriaObj = search.create({
					type: 'customrecord_wmsse_serialentry',
					filters: filterssertemp,
					columns: columns
						...
						if (serialMatchFound) {
							...
							if (inventoryStatusFeature) {
								compSubRecord.setCurrentSublistValue({
									sublistId: 'inventoryassignment',
									fieldId: 'inventorystatus',
									value: SrchRecordTmpSerial1[n].custrecord_serial_inventorystatus
								});
								compSubRecord.setCurrentSublistValue({
									sublistId: 'inventoryassignment',
									fieldId: 'toinventorystatus',
		...
		function getPickBinDetailsLotWithExpiryDates(getItemInternalId, vBinIdArr, getPreferBin, strLocation,
			makeInvAvailFlagFromSelect, itemUnitType, itemStockUnit, allowAllLots, selectedConversionRate, stockConversionUnitname, selectedUOMText, batchno, itemType, locUseBinsFlag) {
		...
		function getCyclePlanTaskDetails(planId, whLocationId, binName, itemInternalId, hideCompletedTaks, reconcilecountZeroqty, noBinExcepforReconcile, locUseBinsFlag) {
			...
			if (utility.isValueValid(hideCompletedTaks))
				getCycPlanTasks.filters.push(
					search.createFilter({
						name: 'quantity',
						operator: search.Operator.ISEMPTY,
					}));
			if (utility.isValueValid(noBinExcepforReconcile))
				getCycPlanTasks.filters.push(
					search.createFilter({
						name: 'quantity',
						operator: search.Operator.ISEMPTY,
					}));
			if (utility.isValueValid(reconcilecountZeroqty))
				getCycPlanTasks.filters.push(
					search.createFilter({
						name: 'quantity',
		...
		function updateCycleCountOpenTask(cyclePlanInternalId, itemInternalId, lineNum, scannedQty,
			...
			if (utility.isValueValid(cyclePlanId))
				objRecord.setValue({
					fieldId: 'name',
					value: cyclePlanId
				});
		...
		function fnGetAllSerialsbyStatus(poInternalId, poLineno, vstatus, vAllSerialArray) {
			...
			if (utility.isValueValid(poLineno))
				serialSearch.filters.push(search.createFilter({
					name: 'custrecord_wmsse_ser_ordline',
					operator: search.Operator.EQUALTO,
					values: poLineno
				}));
			if (utility.isValueValid(poInternalId))
				serialSearch.filters.push(search.createFilter({
					name: 'custrecord_wmsse_ser_ordno',
					operator: search.Operator.ANYOF,
					values: poInternalId
				}));
			if (utility.isValueValid(vstatus))
				serialSearch.filters.push(search.createFilter({
					name: 'custrecord_serial_inventorystatus',
					operator: search.Operator.ANYOF,
					values: vstatus
		...
		function postInventoryStatusChange(inputObj) {
			...
			if (((inputObj.itemType == "inventoryitem" || inputObj.itemType == "assemblyitem") && utility.isValueValid(inputObj.binInternalId)) ||
				inputObj.itemType == "lotnumberedinventoryitem" || inputObj.itemType == "lotnumberedassemblyitem") {
		...
		function getNonStorageBins(cycleCountPlanLocation) {
			...
			nonStoragebinsLoadQuery.columns = [
			...
			nonStoragebinsLoadQuery.sort = [
				nonStoragebinsLoadQuery.createSort({
		...
		function processSelectedGenerateAndReleaseLines(request, response, lineCount, oldResponseArr) {
			...
			if (request.parameters.custpage_hiddenfieldselectpage == 'F') {
				...
				for (var count = 0; count < oldResponseArrlength; count++) {
					...
					if (utility.isValueValid(oldResponseArr[count]))
						currenRecordValue = oldResponseArr[count].split(',');
			...
			if (SrchRecordTmpSerial1.length > 0) {
				...
				if (serialDetailsArr.length > 0) {
					...
					if (serialStatusArr.length > 0) {
						for (var statsItr = 0; statsItr < serialStatusArr.length; statsItr++) {
							var openTaskId = updateCycleCountOpenTask(cyccPlanData.cyccPlanInternalId,
								cyccPlanData.itemInternalId, cyccPlanData.lineNum,
								Number(Big(serialQtyArr[statsItr]).toFixed(8)), cyccPlanData.binInternalId, cyccPlanData.itemType,
								cyccPlanData.warehouseLocationId, '', cyccPlanData.cyccPlanInternalId,
								cyccPlanData.cyclecountPlan, cyccPlanData.actualBeginTime,
								cyccPlanData.unitType, cyccPlanData.conversionRate, 0, '', serialStatusArr[statsItr]);
