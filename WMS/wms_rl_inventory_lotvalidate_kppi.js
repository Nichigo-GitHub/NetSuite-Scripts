 ...
 * @NModuleScope Public
 */
define(['N/search', './wms_utility_kppi', './big_open', './wms_translator_kppi', 'N/format'],
	/**
	 * @param {search} search
	 */
			...
			try {
				if (utility.isValueValid(requestBody)) {
					...
					if (warehouseLocationId.indexOf(':') !== -1) {
						var locationName = warehouseLocationId.split(':')[1];
					} else {
						var locationName = warehouseLocationId;
					}
					log.debug({
						title: 'Location Name',
					...
					if (locationName) {
						...
						var locationResults = locationSearch.run().getRange({ start: 0, end: 1 });
						log.debug({
							title: 'Location Search Results',
							details: locationResults
						 });
						if (locationResults && locationResults.length > 0) {
							warehouseLocationId = parseInt(locationResults[0].getValue({ name: 'internalid' }));
						}
					}
