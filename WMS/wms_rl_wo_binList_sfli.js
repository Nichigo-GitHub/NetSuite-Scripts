 ...
 * @NModuleScope Public
 */
define(['N/search', './wms_utility_sfli', './wms_translator_sfli', './big_open', 'N/config', './wms_workOrderUtility_sfli'],
		...
		function doPost(requestBody) {
			...
			try {
				if (utility.isValueValid(requestBody)) {
					...
					var department = loadConfig.getValue({
						fieldId: 'departments'
					});
					var classes = loadConfig.getValue({
						fieldId: 'classes'
					});
					var internalIdArr = [];
					var objWOLineDtl = {};
					...
					if (utility.isValueValid(warehouseLocationId) && utility.isValueValid(itemInternalId) && utility.isValueValid(itemType)) {
						...
						if (itemDetails.length > 0) {
							...
							log.debug({
								title: 'pickBinDetails',
								details: pickBinDetails
							});
							var itemObjDtl = {};
							...
							if (objBinDetails.length > 0) {
								...
								log.debug({
									title: 'binInternalIdArr',
									details: binInternalIdArr
								});
								...
								log.debug({
									title: 'openTaskPickBinDtls',
									details: openTaskPickBinDtls
								});
