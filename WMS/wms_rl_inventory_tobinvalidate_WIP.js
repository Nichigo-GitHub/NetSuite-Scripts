 ...
 * @NModuleScope public
 */
define(['N/search','N/record','./wms_utility','./big','./wms_translator','./wms_inventory_utility_WIP'],
		/**
		 * @param {search} search
		 */
	...
	function doPost(requestBody) {
				...
				else
				{
					...
					if(processType == 'BinTransfer')
					{
						...
						if(stageLocArr.length > 0)
						{
							...
							log.debug({title:'stgDirection',details:stgDirection});
					...
					log.debug({title:'toBinInternalLoctype '+ toBinInternalId ,details:toBinInternalLoctype});
					...
					{								
						...
						binValidateArray.isValid = false;
						isValidBin = false;
						log.debug({title:'isValidBin',details:isValidBin});
					}
					else
					{
					...
					{	
						...
						if ((utility.isValueValid(fromBinName) || utility.isValueValid(fromLocUseBinsFlag)) && utility.isValueValid(binName) &&
									...
									(blnMixItem == true || blnMixItem == 'true'))
							{
								...
								{
										impactRec = fnPutawayallBinTransfer(itemType, warehouseLocationId, itemInternalId, fromBinInternalId, toBinInternalId, actualBeginTime, stockUnitName, stockConversionRate, lotInternalId, lotName,statusInternalId,processType);
						...
						if (utility.isValueValid(scannedQuantity) && (utility.isValueValid(fromBinName) || utility.isValueValid(fromLocUseBinsFlag)) && utility.isValueValid(binName) &&
							...
							if(utility.isValueValid(binName))
							{
										...
										(blnMixItem == true || blnMixItem == 'true'))
								{
									...
									log.debug({title:'binTransferQty',details:binTransferQty});
									...
									if(processType=='inventoryTransfer')
									{
										...
										impactRec = fnInvTransfer(itemType, warehouseLocationId, toWarehouseLocationId, itemInternalId, binTransferQty, fromBinInternalId,
												toBinInternalId, lotName, actualBeginTime, stockUnitName, stockConversionRate, openTaskQty,fromStatusInternalId, statusInternalId, tallyScanObj);
									...
									else
									{
										...
										impactRec = fnBinTransfer(itemType, warehouseLocationId, itemInternalId, binTransferQty, fromBinInternalId, toBinInternalId, lotName, actualBeginTime, stockUnitName, stockConversionRate, openTaskQty,fromStatusInternalId, statusInternalId,tallyScanObj, processType, sheetQty, manpower, setupStart, setupEnd);
		...
		function fnPutawayallBinTransfer(itemType, warehouseLocationId, itemInternalId, fromBinInternalId, toBinInternalId, actualBeginTime, stockUnitName, stockConversionRate, lotInternalId, lotName,toStatusInternalId,processType)
	{
		var bintransferObj ={};
		var impactRec = {};
		...
		if(utility.isInvStatusFeatureEnabled())
		{
			...
			bintransferObj.toStatusInternalId=toStatusInternalId;
		}
		else
		{
	...
	function fnInvTransfer(itemType, warehouseLocationId, toWarehouseLocationId, itemInternalId, binTransferQty, fromBinInternalId, toBinInternalId, lotName, actualBeginTime, stockUnitName, stockConversionRate, openTaskQty,fromStatusInternalId, statusInternalId, tallyScanObj)
	{
		var invtransferObj ={};
		var impactRec = {};
	...
	function fnBinTransfer(itemType, warehouseLocationId, itemInternalId, binTransferQty, fromBinInternalId, toBinInternalId, lotName, actualBeginTime, stockUnitName, stockConversionRate, openTaskQty, fromStatusInternalId, statusInternalId, tallyScanObj, processType, sheetQty, manpower, setupStart, setupEnd)
	{
		var bintransferObj ={};
		var impactRec = {};
		...
		bintransferObj.processType =processType;
		bintransferObj.sheetQty = sheetQty;
		bintransferObj.manpower = manpower;
		bintransferObj.setupStart = setupStart;
		bintransferObj.setupEnd = setupEnd;
		...
		if(utility.isInvStatusFeatureEnabled())
		{
			bintransferObj.frominvtstatus=fromStatusInternalId;
			bintransferObj.statusInternalId=statusInternalId;
