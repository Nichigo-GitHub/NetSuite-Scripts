						...
						if (!utility.isValueValid(toBinInternalId) || (processType == 'BinTransfer' && toBinInternalLoctype == 'Stage' && fromBinLocationType != null && fromBinLocationType.length == 0)) {
							...
							if (binName != preferedBinName) {
								if (blnMixItem == false || blnMixItem == "false") {
									...
									/* objInvDetails2 = getItemWiseLotsDetails(itemInternalId, warehouseLocationId, toBinInternalId, binName, binValidateArray)

									log.error({
										title: 'objInvDetails2.1',
										details: objInvDetails2
									}) */

									if (objInvDetails.length > 0 || !objInvDetails2) {
									...
									for (var objLotIndex in objInvDetails2) {
										log.error({
											title: 'objInvDetails2[objLotIndex].inventorynumberText',
											details: objInvDetails2[objLotIndex].inventorynumberText
										})							
									} */
