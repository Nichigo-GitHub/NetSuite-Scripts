 ...
 * @NModuleScope public
 */
define(['./wms_utility_sfli', 'N/search', 'N/runtime', 'N/record', 'N/config', 'N/format', './big_open', './wms_translator_sfli', 'N/wms/recommendedBins', 'N/task', 'N/query'],
	...
	return vBinLocArr;
		...
		function createOpenTaskForWorkOrder(openTaskObj) {
			...
			try {
				...
				if (taskType == 'PICK') {
					...
					openTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_pick_comp_date',
						value: parsedPickDate
					});
				} else {
					openTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_wms_status_flag',
						value: openTask_statusFlag
					});
					openTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_tasktype',
						value: openTask_taskType
					});
				}
				if (utility.isValueValid(opentTask_NSConfirmationRefNo)) {
					openTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_nsconfirm_ref_no',
						value: opentTask_NSConfirmationRefNo
					});
				}
				...
				if (utility.isValueValid(openTaskTransactionRefNo)) {
					openTaskRecord.setValue({
						...
						value: openTaskTransactionRefNo
					});
				}
				...
				if (itemType == "serializedinventoryitem" || itemType == "serializedassemblyitem") {
					...
					if (utility.isValueValid(serialArray.serialNumArray)) {
						openTaskRecord.setValue({
							...
							value: serialArray.serialNumArray
						});
					}
					if ((utility.isValueValid(serialArray.serialInternalIdArray)) && taskType == 'PICK') {
						openTaskRecord.setValue({
							fieldId: 'custrecord_wmsse_multi_bins',
							value: serialArray.serialInternalIdArray
						});
					}
				}
		...
		function updateMoveOpenTask(opentaskObj) {
			...
			log.debug({
				title: 'actualBeginTime',
				details: actualBeginTime
			});
			if (utility.isValueValid(actualBeginTime)) {
				var parsedBeginTime = parseTimeString(actualBeginTime);
				customrecord.setValue({
		...
		function getStagedTaskDetails(stageTaskDetails) {
			...
			log.debug({
				title: 'stageTaskDetails',
				details: stageTaskDetails
			});
			var openTaskSearch = search.load({
				id: 'customsearch_wms_wo_comp_stagedetails'
			});
			...
			log.debug({
				title: 'objopentaskSearchDetails',
				details: objopentaskSearchDetails
			});
		...
		function showNextPickTaskButton(transactionName, warehouseLocationId, transactionInternalId, workorderOverpickingFlag) {
			...
			if (utility.isValueValid(woItemListResults) && woItemListResults.length > 0) {
				log.debug({
					title: 'woItemListResults Results :',
					details: woItemListResults.length
				});
				...
				if (utility.isValueValid(openTaskQtyRes) && openTaskQtyRes.length > 0) {
					log.debug({
						title: 'openTaskQtyRes Results :',
						details: openTaskQtyRes
					});
		...
		function isInventoryNumberExists(item, serial, location) {
			...
			var objDetailsSearch = search.load({
				id: 'customsearch_wms_wo_assembly_serialscan'
			});
			...
			for (var index in splitOpenTaskResults) {
				if (runtime.getCurrentScript().getRemainingUsage() <= 100) {
					...
					for (var openTaskItr = 0; openTaskItr < openTaskList.length && assemblyBuildQty > 0; openTaskItr++) {
						...
						var recordActualQty = transactionRec.getValue({
							fieldId: 'custrecord_wmsse_act_qty'
						});
						var recordActualSerail = transactionRec.getValue({
							fieldId: 'custrecord_wmsse_serial_no'
						});
						var recordStatus = transactionRec.getValue({
							fieldId: 'custrecord_wmsse_inventorystatus'
						});
						var recordkBatchNum = transactionRec.getValue({
							fieldId: 'custrecord_wmsse_batch_num'
						});
						var recordActualSerialIds = transactionRec.getValue({
							fieldId: 'custrecord_wmsse_multi_bins'
						});
						if (utility.isValueValid(openTaskStatus) && openTaskStatus.length > 0 &&
							...
							if (((openTaskStatus.indexOf(recordStatus) != -1) && (openTaskLot == '' || openTaskLot == null || openTaskLot == 'null')) ||
								...
								if (parseFloat(assemblyBuildQty) < parseFloat(recordActualQty)) {
									...
									transactionRec.setValue({
										fieldId: 'custrecord_wmsse_expe_qty',
										value: Number(parseFloat(assemblyBuildQty).toFixed(8))
									});
									transactionRec.setValue({
										fieldId: 'custrecord_wmsse_act_qty',
										value: Number(parseFloat(assemblyBuildQty).toFixed(8))
							...
							if ((openTaskLot == '' || openTaskLot == null || openTaskLot == 'null') ||
								...
								assemblyBuildQty = Big(assemblyBuildQty).minus(recordActualQty);
								log.debug({
									title: 'assemblyBuildQty1',
									details: assemblyBuildQty
								});
							}
						}
					}
		...
		function fetchingComponentsforStaging(woInternalId) {
			...
			myOpenTaskQuery.sort = [myOpenTaskQuery.createSort({
				column: myOpenTaskQuery.columns[6]
			})];
		...
		function updateOpenTaskInWOStaging(opentaskSearchResultsPick, binTransferId, stageBinInternalId) {
			...
			vopentaskRec.setValue({
				fieldId: 'custrecord_wmsse_nstrn_ref_no',
				value: binTransferId
			});
			vopentaskRec.setValue({
				fieldId: 'custrecord_wmsse_stagebinloc',
				value: stageBinInternalId
			});
			vopentaskRec.setValue({
				fieldId: 'custrecord_wmsse_actendloc',
				value: stageBinInternalId
		...
		function woOverpickingFlag(workOrderId, whLocation) {
			try {
				...
				if (utility.isValueValid(workOrderId)) {
					var workOrderDetails = record.load({
						type: 'workorder',
						id: workOrderId
					});
					var workCenter = workOrderDetails.getValue({
						fieldId: 'custbody_mfgmob_workcenter'
					});
					var iswip = workOrderDetails.getValue({
						fieldId: 'iswip'
					});
		...
		function nextPickTaskCheckForWOoverpicking(transactionInternalId, woItemListResults) {
			...
			log.debug({
				title: 'nextPickTaskFlagForOverpicking :',
				details: nextPickTaskFlagForOverpicking
			});
		...
		function checkMFGbundleExistOrNot() {
			...
			try {
				...
				log.error({
					title: 'error in  checkMFGbundleExistOrNot',
					details: e
				});
			}
			log.debug({
				title: 'mfgBundleExist',
				details: mfgBundleExist
			});
		...
		function moveOpentaskRecordToClosedTask(openTaskdetails, binTransferId) {
			try {
				...
				closedTaskRecord.setValue({
					fieldId: 'name',
					value: openTaskdetails.name
				});
				closedTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_task_opentaskid_clt',
					value: openTaskdetails.id
				});
				...
				closedTaskRecord.setValue({
					...
					value: openTaskdetails.custrecord_wmsse_act_qty
				});
				if (utility.isValueValid(openTaskdetails.custrecord_wmsse_batch_no))
					closedTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_batch_no_clt',
						value: openTaskdetails.custrecord_wmsse_batch_no
					});
				if (utility.isValueValid(openTaskdetails.custrecord_wmsse_comp_id))
					closedTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_comp_id_clt',
						value: openTaskdetails.custrecord_wmsse_comp_id
					});
				if (utility.isValueValid(openTaskdetails.custrecord_wmsse_currentdate))
					closedTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_currentdate_clt',
						value: openTaskdetails.custrecord_wmsse_currentdate
					});
				if (utility.isValueValid(openTaskdetails.custrecord_wmsse_sku))
					closedTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_sku_clt',
						value: openTaskdetails.custrecord_wmsse_sku
					});
				closedTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_skudesc_clt',
					value: openTaskdetails.custrecord_wmsse_skudesc
				});
				closedTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_expe_qty_clt',
					value: openTaskdetails.custrecord_wmsse_expe_qty
				});
				closedTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_line_no_clt',
					value: openTaskdetails.custrecord_wmsse_line_no
				});
				...
				closedTaskRecord.setValue({
					...
					value: openTaskdetails.custrecord_wmsse_batch_num
				});
				closedTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_compitm_expqty_clt',
					value: openTaskdetails.custrecord_wmsse_compitm_expqty
				});
				closedTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_compitm_actqty_clt',
					value: openTaskdetails.custrecord_wmsse_compitm_actqty
				});
				closedTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_multi_bins_clt',
					value: openTaskdetails.custrecord_wmsse_multi_bins
				});
				closedTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_uom_clt',
					value: openTaskdetails.custrecord_wmsse_uom
				});
				if (utility.isValueValid(openTaskdetails.custrecord_wmsse_conversionrate))
					closedTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_conversionrate_clt',
						value: openTaskdetails.custrecord_wmsse_conversionrate
					});
				closedTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_orderindex_clt',
					value: openTaskdetails.custrecord_wmsse_orderindex
				});
				if (utility.isValueValid(openTaskdetails.custrecord_wmsse_task_assignedto))
					closedTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_task_assignedto_clt',
						value: openTaskdetails.custrecord_wmsse_task_assignedto
					});
				closedTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_orderindex_clt',
					value: openTaskdetails.custrecord_wmsse_orderindex
				...
				if (utility.isValueValid(openTaskdetails.lastmodified))
					closedTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_lastmodified_clt',
						value: openTaskdetails.lastmodified
					});
				closedTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_notes_clt',
					value: "WorkOrder PickReversal"
				});
				if (utility.isValueValid(openTaskdetails.custrecord_wmsse_inventorystatus))
					closedTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_inventorystatus_clt',
						value: openTaskdetails.custrecord_wmsse_inventorystatus
					});
				if (utility.isValueValid(openTaskdetails.custrecord_wmsse_stagebinloc))
					closedTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_stagebinloc_clt',
						value: openTaskdetails.custrecord_wmsse_stagebinloc
					});
				if (utility.isValueValid(openTaskdetails.custrecord_wmsse_pick_comp_date))
					closedTaskRecord.setValue({
						fieldId: 'custrecord_wmsse_pick_comp_date_clt',
						value: openTaskdetails.custrecord_wmsse_pick_comp_date
					});
				if (utility.isValueValid(binTransferId)) {
					closedTaskRecord.setValue({
