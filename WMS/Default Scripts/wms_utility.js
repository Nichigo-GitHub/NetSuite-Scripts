...
//		end
            ...
            if (isBarcodeRuleEnabled){
                ...
                if (getSystemRuleValueWithProcessType('Enable Advanced Barcode Scanning?', location, translator.getTranslationString("ADVANCE_BARCODE.HIBCBARCODE")) == 'Y' && itemNo[0] == '+' && !flag) {
                    ...
                    if (_isValueValid(itemfrmBarcode.barcodeVendor)){
                        ...
                        var vendorResults = vendorSearchObj.run().getRange({start:0,end:1});
	...
	function validateItemWithGS1ItemAlias(itemNo, location, searchItem,vendor){
		...
		return { 'itemResults' : itemResults , 'itemResultObj' : itemResultObj};
	}
	...
	function getSystemRuleValueWithProcessType(RuleId, loc,transactionType ) {
		var systemRuleValue = '';
		log.debug({title: 'transactionType',details:transactionType});
		...
		try {
			...
			if (_isValueValid(searchresults) && searchresults.length > 0) {
					...
					var systemRuleProcessType=systemRuleRecord.getValue({name:'custrecord_wmsseprocesstype'});
					var location=systemRuleRecord.getValue({name:'custrecord_wmssesite'});
                    var systemRuleProcessTypeText = systemRuleRecord.getText({name:'custrecord_wmsseprocesstype'});
					...
					if((location && location==loc))
					{
						...
						{
                            systemRuleValue= systemRuleRecord.getValue({name:'custrecord_wmsserulevalue'});
	...
	function getCurrentTimeStamp(){
		...
		var now = convertDate();
	    var dateString = format.format({value: now,
	            type: format.Type.DATETIME});
			var timeFormatFromPref = TimeSetting();
			log.debug({title:'timeFormatFromPref',details:timeFormatFromPref});
			if(timeFormatFromPref.lastIndexOf("a") != -1)
			{
				dateString = dateString.substring(0,dateString.lastIndexOf(" "));
			}
		var timeStampIndex = dateString.lastIndexOf(" ");
        var dateTimeString = format.format({value: now,
            type: format.Type.DATETIME});
	...
	function getInventoryStatusOptions()
	{
		...
		if(statusList.length > 0)
		{
			for ( var statusItr in statusList)
			{
				...
				var row1={'internalid':listID,'name':listValue, 'listInventoryavailable':listInventoryavailable};
				sOptionsArr.push(row1);
			}
		}
		return sOptionsArr;
	...
	function updateScheduleScriptStatus(processname,currentUserId,status,transactionInternalId,transactionType,notes,pickActionIdArray)
	{
		...
		{
			var now = new Date()
			var datetime =format.parse({value: now, type: format.Type.DATETIMETZ})
		...
		else if(status=='In Progress')
		{
			...
			if(statusSearchResult!=null && statusSearchResult!='')
			{
				...
				var schStatusRecord = record.load({type : 'customrecord_wmsse_schscripts_status',
					                              id   :  vid
					                             });
                schStatusRecord.setValue({fieldId :'custrecord_wmsse_schprsstatus',value: status});
                schStatusRecord.setValue({fieldId :'custrecord_wmsse_schprsbegindate',value: parsedCurrentDate});
                schStatusRecord.setValue({fieldId :'custrecord_wmsse_schprsbegintime',value: parsedCurrentTime});
		...
		else if(status=='Completed')
		{
			...
			if(statusSearchResult!=null && statusSearchResult!='')
			{
                ...
                schStatusRecord.setValue({fieldId :'custrecord_wmsse_schprsstatus',value: status});
                schStatusRecord.setValue({fieldId :'custrecord_wmsse_schprsenddate',value: parsedCurrentDate});
                schStatusRecord.setValue({fieldId :'custrecord_wmsse_schprsendtime',value: parsedCurrentTime});
                schStatusRecord.setValue({fieldId :'custrecord_wmsse_schprsnotes',value: notes});
                schStatusRecord.save();
	...
	function fnGetInventoryBinsForLotForBinPutaway(strLocation,strLot,ItemInternalId,binnumber) {
		...
		log.debug({title:'lotresultsbinLocArr',details:binLocArr});
		return binLocArr;
	}
...
//			log.debug({title:'objBinPreferBinDetails',details:objBinPreferBinDetails});
	...
	function _fnGetItemInventoryBins(strLocation,ItemInternalId) {
		...
		if (itemBinDetails.length > 0) {
			for (var f = 0; f < itemBinLength; f++) {
				var itemBinId = itemBinDetails[f].getValue({name:'binnumber',summary: 'group'});
				if (_isValueValid(itemBinId) && binLocArr.indexOf(itemBinId) == -1) {
					binLocArr.push(itemBinId);
				}
				var obj = {
						'status':itemBinDetails[f].getText({name:'status',summary: 'group'}),
						'statusId':itemBinDetails[f].getValue({name:'status',summary: 'group'}),
						'quantity':itemBinDetails[f].getValue({name:'onhand',summary: 'sum'})};
				binArray.push(obj);
	...
	function _getputStratagiePreferBinDetails(preferBinInternalId,strLocation,fromBinInternalId,fromStatusId,objPutBinQueryDetails)
	{
		...
		if (objBinPreferBinDetails.length > 0) {
			...
			{
				var strBin = objBinPreferBinDetails[p].getText({name:'binnumber',summary:'group'});
				var strBinId = objBinPreferBinDetails[p].getValue({name:'binnumber',summary:'group'});;
				var invStatus = objBinPreferBinDetails[p].getText({name:'status',summary:'group'});
				var invStatusId = objBinPreferBinDetails[p].getValue({name:'status',summary:'group'});
				var vBinQtyAvail = objBinPreferBinDetails[p].getValue({name:'onhand',summary:'sum'});
		...
		log.debug({title:'preferBinDetailsObj[p]',details:preferBinDetailsObj});
	...
	function _getPutStratagies(getItemInternalId,strItemGrp,strItemFam,strLocation,strItemClass,strItemDepartment,strvUnits,fromStatusId,inventoryStatusFeature,classification)
	{
		...
		if(inventoryStatusFeature)
		{
			...
			{
				filters.push(search.createFilter({
					name: 'custrecord_wmsse_put_invstatus', operator: search.Operator.ANYOF,
		...
		if(_isValueValid(classification))
		{
			...
			{
				filters.push(search.createFilter({
					name: 'custrecord_wmsse_abcvelocity', operator: search.Operator.ANYOF,
		...
		if(itemBinsArray.length > 0)
		{
			...
			if(!mixItemsInBins)
			{
				if(itemBinArr.length > 0)
				{
					...
					log.debug({title:'itemBinArr after mixflag',details:itemBinArrAfterMixFlagChk});
...
function _getBinsByZoneAndLocation(mixItemsInBins,putZoneArr,strLocation,existingBinArrayLength,binInternalIdArr,putStrategyArr,selectedConvRate, currentConvRate, selectedUOMText,fromstausId,inventoryStatusFeature)
	...
	function _getInventoryBalanceDetailsByZoneAndLoc(strLocation,binInternalIdArr,existingBinArrayLength)
	{
		...
		function getPutBinAndIntDetails(objPutBinQueryDetails){
			...
			var binDetailsArray =[];
			var tdate =  new Date();
			log.debug({title:'timeStamp at the start of the getPutBinAndIntDetails function',details:tdate.getHours()+":"+tdate.getMinutes()+":"+tdate.getSeconds()+":"+tdate.getMilliseconds()});
			...
			log.debug({title:'fromStatusId',details:fromStatusId});
			var inventoryStatusFeature = isInvStatusFeatureEnabled();
			var vTotalBinArr = [];
			if(_isValueValid(preferBinInternalId) && preferBinInternalId != fromBinInternalId){
			...
			log.debug({title:'objPutstrategies',details:objPutstrategies});
			...
			if (objputstrategiesLength > 0) {
				...
				log.debug({title:'putZoneArr',details:putZoneArr});
				log.debug({title:'putStrategyArr',details:putStrategyArr});
		...
		function _getInventoryBinsForMixFlagFalseItem(whLocationId,itemInventoryResults){
			...
			if(itemBinsArr.length > 0) {
				var objsearchObj = search.load({
					id: 'customsearch_wms_getitemdtls', type: "item"
				});
				var filters = objsearchObj.filters;
				filters.push(search.createFilter({
					name: 'binnumber', join: 'binOnHand', operator: search.Operator.ANYOF,
					values: itemBinsArr
				}));
				filters.push(search.createFilter({
					name: 'location', join: 'binOnHand', operator: search.Operator.ANYOF,
					values: whLocationId
				}));
		...
		function _getInventoryBinsForItem(whLocationId,putZoneArr,itemId,fromBinInternalId,fromStatusId,preferBinInternalId,strLotID){
			...
			var objItemSearchObj = search.load({
				id: 'customsearch_wms_mixedinventorybinsdtls',type:"inventorybalance"
			});
			...
			if(_isValueValid(strLotID)) {
				...
				filters.push(search.createFilter({
					name: 'inventorynumber', operator: search.Operator.ANYOF,
					values: strLotID
				}));
			}
			if(_isValueValid(fromStatusId) && !isNaN(fromStatusId)) {
			...
			{
				filters.push(search.createFilter({
					name: 'custrecord_wmsse_zone',join:'binnumber', operator: search.Operator.ANYOF,
					values: putZoneArr
				}));
			}
			if(_isValueValid(itemId)) {
				filters.push(search.createFilter({
					name: 'item', operator: search.Operator.ANYOF,
					values: itemId
				}));
			}
		...
		function fillInvBalanceDtls(mixItemsInBins,itemInventoryResults,selectedConvRate,currentConvRate,selectedUOMText,putZoneArr,putStrategyArr,binArr){
			...
			for ( invBalanceBin=0; invBalanceBin < invBalanceLength; invBalanceBin++) {
				...
				var currentRowValues = {'binName':invBalanceBinName,'binInternalId':invBalanceBinId,'statusName':invBalanceStatus,
					'statusInternalId':invBalanceStatusId,'quantity':invBalanceQuantity,'zone':invBalanceZone,'quantityWithUOM':invBalanceUOMQuantity,
					'sortByZoneIndex':putZoneIndex,'sortBySeqIndex':invBalanceBin,'puStratagie':putstrategy};
	...
	function getPutBinAndIntDetails_old(objPutBinQueryDetails)
	{
			...
			for (var i = 0; i < objPutstrategies.length && parseInt(binCountVariable) <= 4000; i++) {
				...
				if (strPutZone != null && strPutZone != '' && vPutZoneArr.indexOf(strPutZone) == -1) {
					...
					if (binLength > 0) {
						...
						log.debug({title:'objInvBalanceBinDetails',details:objInvBalanceBinDetails});
							...
							if(invBalanceObject != undefined)
							{
								...
								for(invObj=0 ;invObj < invBinObjectLength; invObj++)
								{
											...
											||	(inventoryStatusArr.indexOf(invStatusId) != -1 || fromStatusId == 'All'))
									{
										...
										{
											var currentRowValues = {'binName':strBin,'binInternalId':strBinId,'statusName':invStatus,
													'statusInternalId':invStatusId,'quantity':binQtyAvail,'zone':zone,'quantityWithUOM':binQtyAvailWithUOM,
													'sortByItem':sortVar,'sortBySeq':bin,'puStratagie':objPutstrategies[i].id};
		...
		var tdate =  new Date();
		log.debug({title:'timeStamp at the end of the  function',details:tdate.getHours()+":"+tdate.getMinutes()+":"+tdate.getSeconds()+":"+tdate.getMilliseconds()});
		return vTotalBinArr;
	...
	function getItemMixFlagDetails(warehouseLocationId, itemInternalId, binInternalId, mixItem, mixLot){
		...
		if(mixLot){
			searchObj.filters.push(search.createFilter({ name :'custitem_wmsse_mix_lot',
				operator: search.Operator.IS,
				values: false
			}));
		}
		if(mixItem){
			searchObj.filters.push(search.createFilter({ name :'custitem_wmsse_mix_item',
				operator: search.Operator.IS,
				values: false
			}));
		}
...
//new GUI post item receipt
			...
			if(key == 'barcodeUomName' || key == 'barcodeQuantity' || key == 'itemUomName')
			{
				...
				if(uomValue != '')
				{
					if(_isValueValid(uomQty)){
						var uomQtyObj = [{'value':uomQty,'unit':uomValue}];
						itemList['barcodeQuantity'] = uomQtyObj;
					}
					else{
						var uomQtyObj = [{'value':'','unit':uomValue}];
						itemList['barcodeQuantity'] = uomQtyObj;
					}
				}
				else
	...
	function getDefaultInvStatus(){
		...
		var resultSuiteQL= query.runSuiteQL({query: invStatusQuery,
			params : ['F']
		});
	...
	function getOpenTaskPickBinDetailsQuery(itemIds, binIds, warehouseLocationId, inventoryStatusFeature,
		...
		log.debug({title:'objOPentaskDetails.length --',details:objOPentaskDetails.length});
		...
		if (_isValueValid(vUnits)) {
			...
			if (uomresults != null && uomresults != '') {
				...
				for (var i = 0; i < uomresults.length; i++) {
					vvFromRate = uomresults[i].getValue({name: 'conversionrate'});
					var unitId = uomresults[i].getValue({name: 'internalid'});
					var unitname = uomresults[i].getValue({name: 'unitname'});
	...
	function getDefaultRuleValueForAsyncSystemRules(systemRuleName)
	{
		...
		for(var arrIndex = 0; arrIndex < sytemrulesArray.length;arrIndex++){
			...
			systemRuleLookup[sytemrulesArrayKey] = { ruleValue: sytemrulesArrayValue };
		}
...
//log.debug('fetched IFperorder in utlity for first time',IFperorder);
			...
			for(arrIndex =0;arrIndex < wmsPreferencesLength; arrIndex++)
			{
				...
				if(wmsPreferencesDetails.length==0)
				{
					...
					wmsPreferencesRecord.setValue({ fieldId :'name',value :preferencesName});
					wmsPreferencesRecord.setValue({ fieldId :'custrecord_wms_accountingpre_name',value :preferencesName});
					...
					{
						wmsPreferencesRecord.setValue({ fieldId :'custrecord_wms_accountingpre_value',value :preferencesValue});
					}
					else if(preferencesName =="CREATEITEMFULFILLMENT")
					{
						log.debug('setting values in notes field while creating',IFperorder);
						wmsPreferencesRecord.setValue({ fieldId :'custrecord_wms_accountingpre_notes',value :IFperorder});
					}
						else if(_isValueValid(fisrtReceipientId) || _isValueValid(secondReceipientId))
						{
							wmsPreferencesRecord.setValue({ fieldId :'custrecord_wms_accountingpre_emailreqire',value :true});
						}
	...
	function getWmsPreferencesValue(preferencesName)
	{
		...
		try
		{
			...
			if (resultsArray.length > 0) {
				...
				var tempRecord = record.load({type:'customrecord_wms_accounting_preferences',
					id:recordId,
					isDynamic: true
				});
				tempRecord.save();
    ...
    function logDatatoELK(dataObj){
		...
		function setPaging(sublistDetailArr,form,request)
		{
			...
			var companyPreferencesPageSize = loadConfig.getValue({fieldId:'LISTSEGMENTSIZE'});
    ...
    function getDeparmentandClassSettings()
		{
			...
			var makeDepartmentsMandatory = accountConfig.getValue({fieldId: 'DEPTMANDATORY'});
			var makeClassMandatory = accountConfig.getValue({fieldId: 'CLASSMANDATORY'});
				...
				for (var barcodecomponent in srchbarcodecomponents)
				{
					...
					case 'Expiry Date':
						if(_isValueValid(inputDataObj.lotExpiryDate))
						{
							...
							try{
							var lotExpiryDate = format.parse({ value: inputDataObj.lotExpiryDate,
		                                                 type: format.Type.DATE
                                                    	});
