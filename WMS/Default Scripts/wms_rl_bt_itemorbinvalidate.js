...
define(['N/search','./wms_utility','./wms_translator','./wms_inventory_utility'],
	...
	function doPost(requestBody)
	{
		...
		try{
			if(utility.isValueValid(requestBody))
			{
				...
				if(utility.isValueValid(inputStringVar) && utility.isValueValid(wareHouseLocationId))
				{
					binoritemArray =binValidation(requestParams);
					log.debug({title:'binoritemArray',details:binoritemArray});
					if(binoritemArray.isValid == false || binoritemArray.isValid == 'false')
					{
						var	itemResObj = utility.itemValidationForInventoryAndOutBound(inputStringVar,wareHouseLocationId);


						if(utility.isValueValid(itemResObj.itemInternalId) ||
								utility.isValueValid(itemResObj.barcodeIteminternalid))
						{	
							var itemInternalId= (itemResObj.itemInternalId ? itemResObj.itemInternalId : itemResObj.barcodeIteminternalid);
							var itemName = (itemResObj.itemName ? itemResObj.itemName : itemResObj.barcodeItemname);

							var objBinDetails = getUseBins(itemInternalId,wareHouseLocationId);
							log.debug({title:'objBinDetails',details:objBinDetails});

							if(objBinDetails != null && objBinDetails != '')
							{
								if(objBinDetails != undefined && objBinDetails != 'undefined')
								{
									var useBins =  objBinDetails.usebins;
									log.debug({title:'useBins',details:useBins});
									if(useBins == false)
									{
										binoritemArray.errorMessage = translator.getTranslationString('BINTRANSFER_ITEMVALIDATE.USEBIN_FLAG');
									...
									else
									{
										...
										var itemType = '';
										if(objBinDetails.unitstype[0] != undefined && objBinDetails.unitstype[0] != 'undefined')
										{
										...
										if(objBinDetails.recordtype != undefined && objBinDetails.recordtype != 'undefined')
										{
										...
										if(itemType == '' && objBinDetails.recordtype != undefined)
										{
											itemType = objBinDetails.recordtype;
										}
										...
										binoritemArray	= utility.addItemDatatoResponse(binoritemArray, itemResObj, unitTypeIdNum, stockUnitName);
										...
										{
											binoritemArray.errorMessage = translator.getTranslationString('BINTRANSFER_ITEMORBINVALIDATE.ITEM_VALIDATION');
											binoritemArray.isValid = false;
										}
									}
								}
						...
						{
							if(itemResObj.error === translator.getTranslationString("PO_ITEMVALIDATE.INVALID_INPUT"))
							{
							...
							{
								log.debug({title:'itemResObj.error',details:itemResObj.error});
						...
						if(processType == 'cartPutAway'){
							var loadAllSystemRuleValue = utility.getSystemRuleValue('Enable bulk loading into staging carts',wareHouseLocationId);
							if(utility.isValueValid(loadAllSystemRuleValue) && loadAllSystemRuleValue == 'Y'){
	...
	function binValidation(requestParams)
	{
		...
		{
			if(utility.isValueValid(processType) && (processType == 'putAway' || processType == 'cartPutAway')) {
				binSearchFilters.push(search.createFilter({
		...
		if(binSearchResults.length > 0)
		{
			...
			if(processType == 'binTransfer' && binSearchResults[0].getText({name:'custrecord_wmsse_bin_loc_type'})=='WIP' && binSearchResults[0].getValue({name:'custrecord_wms_mfg_picking'}) == false)
			{
				binInternalId ="";
			}
		}
		...
		{
			if(binSearchResults[0].getText({name:'custrecord_wmsse_bin_loc_type'})=='Stage' && 
					binSearchResults[0].getText({name:'custrecord_wmsse_bin_stg_direction'}) == '')
			{
				binoritemArray.errorMessage = translator.getTranslationString('BINPUTW_FROM_BINVALIDATE.INVALID_INPUT');
				binoritemArray.isValid = false;
			}
			else
			{
				binoritemArray.binInternalId = binInternalId;
				binoritemArray.binName = binSearchResults[0].getValue({ name : 'custrecord_bin_code'});
