								...
								(openTaskLot!='' && openTaskLot !=null && openTaskLot.indexOf(recordkBatchNum) != -1))
						{
								...
								if(utility.isValueValid(recordActualSerail) && utility.isValueValid(openTaskSerial)){
									...
									cloneOpenTaskRecord (assemblyBuildOpenTaskId,recordActualQty,assemblyBuildWiseQuantity,transactionInternalId,
										newSerialString,recordActualSerialIds,newSerialInternalId);								
									transactionRec.setValue({fieldId :'custrecord_wmsse_expe_qty', value: Number(parseFloat(assemblyBuildWiseQuantity).toFixed(8))});
									transactionRec.setValue({fieldId :'custrecord_wmsse_act_qty', value: Number(parseFloat(assemblyBuildWiseQuantity).toFixed(8))});
							}
							transactionRec.setValue({fieldId :'custrecord_wmsse_nsconfirm_ref_no', value: results.assemblyRecId});
							transactionRec.save();
