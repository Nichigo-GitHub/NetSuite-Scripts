 ...
 * @NModuleScope Public
 */
define(['N/search', './wms_utility', './wms_translator', './big', 'N/config', 'N/record', 'N/runtime', './wms_workOrderUtility', 'N/task'],
	...
	function (search, utility, translator, Big, config, record, runtime, workOrderUtility, task) {
				...
				if (utility.isValueValid(requestBody)) {
					...
					if (utility.isValueValid(binInternalId)) {
						if (binInternalId != preferedBinInternalId) {
							if (binMixItem != true) {
								var invBalanceDetails = validateMixItemAndMixLot(assemblyItemInternalId, binInternalId, warehouseLocationId, null, null, assemblyItemType);
							...
							if (isValidBin && binMixLot != true && assemblyItemType == "lotnumberedassemblyitem") {
								var invBalanceDetails = validateMixItemAndMixLot(assemblyItemInternalId, binInternalId, warehouseLocationId, lotName, null, assemblyItemType);
								if (invBalanceDetails.length > 0) {
									isValidBin = false;
									binValidateObj['isValid'] = false;
		...
		function validateWOBin(binName, warehouseLocationId, binValidateObj) {
			...
			if (utility.isValueValid(binName)) {
				var binSearch = search.load('customsearch_wmsse_woqty_bin_srh_2');
				...
				if (binSearchRes.length == 0) {
					...
					binObj.binName = binSearchRes[0]['custrecord_bin_code'];
				}
			} else {
				binValidateObj['isValid'] = false;
				binValidateObj["errorMessage"] = translator.getTranslationString("BINTRANSFER_TOBINVALIDATE.INVALID_BIN");
		...
		function validateMixItemAndMixLot(itemInternalId, binInternalId, warehouseLocationId, inventoryNumber, statusInternalId, itemType) {
			var invBalanceSearch = '';
			if (itemType == "serializedinventoryitem" || itemType == "serializedassemblyitem") {
				invBalanceSearch = search.load({
					type: search.Type.INVENTORY_BALANCE,
					id: 'customsearch_wms_get_invbalance_detail_2'
				});
			} else {

				invBalanceSearch = search.load({
					type: search.Type.INVENTORY_BALANCE,
					id: 'customsearch_wms_wo_invbal_inv_lot_exp_2'
				});
			}
		...
		function splitOpenTaskRecords(splitOpenTaskResults, results, transactionInternalId, inventoryStatusFeature) {
			for (var index in splitOpenTaskResults) {
				if (runtime.getCurrentScript().getRemainingUsage() <= 100) {
					...
					var openTaskInvStatusObject = splitOpenTaskResults[index]['openTaskInvStatusObj'];
					var lotWiseQuantiy = splitOpenTaskResults[index]['lotWiseQuantity'];
					...
					for (var openTaskItr = 0; openTaskItr < openTaskList.length && assemblyBuildQty > 0; openTaskItr++) {
						...
						if (utility.isValueValid(openTaskStatus) && openTaskStatus.length > 0 &&
							...
							if (((openTaskStatus.indexOf(recordStatus) != -1) && (openTaskLot == '' || openTaskLot == null || openTaskLot == 'null')) ||
								...
								if ((parseFloat(assemblyBuildQty) < parseFloat(recordActualQty)) || (parseFloat(openTaskInvStatusQuantity) < parseFloat(recordActualQty))) {
									var newSerialString = '';
							...
							if ((openTaskLot == '' || openTaskLot == null || openTaskLot == 'null') ||
								...
								if ((parseFloat(assemblyBuildQty) < parseFloat(recordActualQty)) || ((parseFloat(assemblyBuildWiseQuantity) < parseFloat(recordActualQty)) &&
										(openTaskLot.indexOf(recordkBatchNum) != -1))) {
									if ((parseFloat(assemblyBuildQty) < parseFloat(recordActualQty)) && (openTaskLot.indexOf(recordkBatchNum) == -1)) {
										assemblyBuildWiseQuantity = assemblyBuildQty;
									}
									var newSerialString = '';
									var newSerialInternalId = '';
									if (utility.isValueValid(recordActualSerail) && utility.isValueValid(openTaskSerial)) {
									...
									transactionRec.setValue({
										fieldId: 'custrecord_wmsse_expe_qty',
										value: Number(parseFloat(assemblyBuildWiseQuantity).toFixed(8))
									});
									transactionRec.setValue({
										fieldId: 'custrecord_wmsse_act_qty',
										value: Number(parseFloat(assemblyBuildWiseQuantity).toFixed(8))
