...
define(['N/search', './wms_utility', './big', './wms_translator', './wms_tallyScan_utility'],
	...
	function (search, utility, Big, translator, tallyScanUtility) {
		...
		function doPost(requestBody) {
            log.debug({
                title: 'doPost - Start',
                details: 'Processing request with body: ' + JSON.stringify(requestBody)
            });
				...
				if (utility.isValueValid(requestBody)) {
                    ...
                    log.debug({
                        ...
                        details: {
                            ...
                            stockConversionRate: getStockConversionRate,
                            preferedBinName: preferedBinName,
                            warehouseLocationId: warehouseLocationId,
                            itemType: itemType,
                            itemInternalId: itemInternalId,
                            unitType: unitType,
                            makeInvtAvailableFlag: makeInvtAvailableFlag,
                            processType: processType,
                            barcodeQuantity: barcodeQuantity,
                            itemAliasUnit: itemAliasUnit
                        }
                    });
					...
					if (utility.isValueValid(fromBinName) && utility.isValueValid(warehouseLocationId) &&
						...
						if (utility.isValueValid(fromBinName)) {
							...
							if (!(utility.isValueValid(binInternalId))) {
                                log.debug({
                                    title: 'Bin Validation Failed',
                                    details: 'Invalid binInternalId: ' + binInternalId
                                });
								//fromBinValidate['errorMessage'] = translator.getTranslationString('BINTRANSFER_FROMBINVALIDATE.INVALID_INPUT');
								fromBinValidate.isValid = false;
							} else {
                                log.debug({
                                    title: 'Bin Validation Success',
                                    details: 'Valid binInternalId: ' + binInternalId
                                });
								var objBinDetails = [];
								...
								if (objBinDetails.length > 0 && availableQty > 0) {
                                    ...
                                    log.debug({
                                        title: 'Tally Scan Check',
                                        details: 'isTallyScanRequired: ' + fromBinValidate.isTallyScanRequired
                                    });
									if (fromBinValidate.isTallyScanRequired) {
                                        ...
                                        log.debug({
                                            title: 'Barcode Quantity Set',
                                            details: 'Final barcodeQuantity: ' + JSON.stringify(barcodeQuantity)
                                        });
									}
									fromBinValidate.barcodeQuantity = barcodeQuantity;
									log.debug({
                                        title: 'Final Response',
                                        details: JSON.stringify(fromBinValidate)
                                    });
								}
							}
						} else {
