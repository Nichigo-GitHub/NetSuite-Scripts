 ...
 * @NModuleScope Public
 */
define(['N/search','N/config','./wms_utility','./big','./wms_translator', './wms_inboundUtility','./wms_inbound_utility','N/record','N/runtime','N/task'],
		/**
		 * @param {search} search
		 */
					...
					else
					{
						...
						if(!(utility.isValueValid(conversionRate))){
							if(utility.isValueValid(itemSearchResults[0]['unitstype']))
								conversionRate = utility.getStockCoversionRate(itemSearchResults[0]['unitstype'],
										uom , '1');
						}
						...
						if(utility.isValueValid(enterBin))
						{
							...
							if((itemType != "noninventoryitem" && itemType != "otherchargeitem" &&
									itemType!="serviceitem" && itemType!="downloaditem" && itemType!="giftcertificateitem") || randomTallyScanRule == 'T')
							{
								if(systemRule=='N')
								{
									...
									if(blnMixItem == false || blnMixLot == false)
									{
										if(enterBin != preferBin && binInternalId != '' && binInternalId != null)
										{
											...
											if((!blnMixLot) && (itemType == translator.getTranslationString("ITEMTYPE_LOT") || itemType==translator.getTranslationString("ITEMTYPE_LOT_ASSEMBLY")))
											{
								...
								else
								{
									...
									if(binSearchResults !=null && binSearchResults !="")
									{
										...
										else if(binSearchResults[0].getValue({name:'custrecord_wmsse_bin_stg_direction'})!=1)
										{
										...
										else
										{
											binInternalId=binSearchResults[0].id;
											binLocType = 'Stage';
											if(utility.isValueValid(binSearchResults[0].getValue({name:'custrecord_wms_iscart'})) && binSearchResults[0].getValue({name:'custrecord_wms_iscart'})==true){
							...
							{
								var orderItemLineDetails =  inboundUtility.getRecevingOrderItemDetails(tranid,fetchedItemId,whLocation,orderLineNo,null,null,transactionType,crossSubsidiaryFeature,centralizesPurchasingandBilling,warehouseLocationName);
								...
								var openPutAwayDetails = inboundUtility.getOpentaskOpenPutwayDetails(orderId,whLocation,fetchedItemId,orderLineNo);
									...
									if(transactionType=='returnauthorization')
										...
										orderRemQty = orderLineDetailsRec.poRemainingQty;
										...
										if (opentaskSearchResults.length > 0) {
											...
											for (var otItr = 0; otItr < opentaskSearchResults.length; otItr++) {
												...
												//	enterBin = opentaskSearchResults[otItr].custrecord_wmsse_actendloc;
													...
													inboundLib.consolidatePostItemReceipt(trecord, actQuantity, linenum, itemId, transactionType, batchNo, expiryDate,
														whLocation, binInternalId, serialArray, opentaskSearchResults, orderInternalId, otItr,randomTallyScanRule);
													//itemIterator++;
											...
											if (itemReceipt != '' || systemRuleValue == 'Y') {
												if((runtime.getCurrentScript().getRemainingUsage() > scriptUsageRequiredToUpdateOpenTask(opentaskSearchResults.length))
													...
													for (var j = 0; j < opentaskSearchResults.length &&
													runtime.getCurrentScript().getRemainingUsage() >= 20; j++) {
								...
								if(binValidateDetails.mapReduceTriggered != "Y") {
									orderLineDetails = inboundUtility.getRecevingOrderItemDetails(tranid, null, whLocation, null, null, null, transactionType, crossSubsidiaryFeature, centralizesPurchasingandBilling, warehouseLocationName);
								}
									if(orderLineDetails !=null && orderLineDetails.length > 0)
								{
									...
									for(var cnt in orderLineDetails)
									{
										...
										if(JSON.stringify(openPutAwayDetails) !== '{}')
										{
											...
											if(utility.isValueValid(itemOpentaskRcvQty))
											{
												...
												if(transactionType=='returnauthorization')
													...
													lineRemainingQty = orderLineArr.poRemainingQty;
										...
										if(transactionType=='returnauthorization')
											...
											orderRemainingqty = orderLineArr.poRemainingQty;
	...
	{
		var itemSearch= search.load({id:'customsearch_wmsse_receivingitemdetails'});
	...
	{
		var invDetailsSearch = search.load({id:'customsearch_wmsse_itemwise_invt_inbound'});
