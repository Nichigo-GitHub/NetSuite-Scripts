 ...
 * @NModuleScope Public
 */
define(['N/search', 'N/log', './wms_utility', './wms_translator', './big', './wms_workOrderUtility_kppi'],
                ...
                if (utility.isValueValid(requestBody)) {
                    ...
                    if (utility.isValueValid(whLocation)) {
                        ...
                        if (utility.isValueValid(transactionName)) {
                            ...
                            var workOrdDtlResults = woUtility.fnToValidateWO_V3(transactionName);
                            log.debug('fnToValidateWO_V3 returned', { length: (workOrdDtlResults && workOrdDtlResults.length), sample: workOrdDtlResults && workOrdDtlResults[0] });
                            ...
                            if (workOrdDtlResults && workOrdDtlResults.length > 0) {
                                ...
                                orderDetails['isValid'] = false;
                                log.debug('fnToValidateWO_V3 returned no results or empty -> INVALID_ORDER');
                            }
                        } else {
                            orderDetails['errorMessage'] = translator.getTranslationString('WORKORDER_PICKING.INVALID_ORDER');
                            orderDetails['isValid'] = false;
