 ...
 * @NModuleScope Public
 */
define(['N/runtime', 'N/search', 'N/record', 'N/query', './wms_utility_kppi', './big_open', './wms_translator_kppi', './wms_inboundUtility_kppi', './wms_tallyScan_utility', './wms_inbound_utility', './wms_labelPrinting_utility'],
	/**
	 * @param {search} search
	 */
	function (runtime, search, record, query, utility, Big, translator, inboundUtility, tallyScanUtility, inboundLib, labelPrintingUtility) {
		...
		function doPost(requestBody) {
			...
			try {
				if (utility.isValueValid(requestBody)) {
					...
					var preparedBy = requestParams.preparedBy; //F_RT
					...
					log.debug('preparedBy', preparedBy);
										...
										if (isTallyScanRequired != true) {
											...
											if ((!utility.isValueValid(numberOftimesSerialScanned) || numberOftimesSerialScanned == 0 ||
												...
												if (randomTallyScanRule == "T") {
													...
													openTaskRecId = _getOpenTask(transactionInternalId, lineNo, inventoryStatusFeature, requestParams.invtStatus, locationUseBinlFlag, '', [], preparedBy);
													var binLoc = "";
												...
												if (serialSearchResultsLength > 0) {
													...
													if (randomTallyScanRule == 'T' && locationUseBinlFlag == true) {
														opentaskSerials = _getSerials(transactionInternalId, lineNo, preparedBy);
													}
											...
											if (randomTallyScanRule == 'T') {
												...
												if (utility.isValueValid(tallyLoopObj[key])) {
													...
													openTaskIdFromTallyLoopObj = _getOpenTask(transactionInternalId, transactionDetailsArray.transactionLineNo,
														inventoryStatusFeature, invtStatus, locationUseBinlFlag, lot, scannedLotArr, preparedBy);
													tallyscanQty = 1;
													if (itemType != "serializedinventoryitem" && itemType != "serializedassemblyitem" &&
														itemType != "lotnumberedinventoryitem" && itemType != "lotnumberedassemblyitem" &&
														itemType != "inventoryitem" && itemType != "assemblyitem") {
												...
												if (!utility.isValueValid(openTaskIdFromTallyLoopObj)) {
													...
													if (utility.isValueValid(locationUseBinlFlag) && locationUseBinlFlag == true) {
														...
														// itemInputObj.drNumber = drNumber;
												...
												log.debug("preparedBy createOrUpdateOpentask wms_inboundUtility_kppi", preparedBy);
												transactionDetailsArray.openTaskId = inboundUtility.createOrUpdateOpentask(openTaskIdFromTallyLoopObj, tallyscanQty, objOpentaskDetails, warehouseLocationId, lotName1, '', preparedBy);
												if (itemType == "serializedinventoryitem" || itemType == "serializedassemblyitem") {
									...
									if ((itemType != "noninventoryitem" && itemType != "otherchargeitem" &&
										...
										if (transactionType == 'transferorder') {
											if (inventoryStatusFeature) {
												...
												// transactionDetailsArray.drNumber = drNumber;
												transactionDetailsArray.isValid = true;
											}
										} else {
											transactionDetailsArray.isValid = true;
										...
										if (itemType == "noninventoryitem" || itemType == "otherchargeitem" ||
											...
											if (!utility.isValueValid(isTallyScanRequired) || isTallyScanRequired == false ||
												...
												if (transactionType == 'transferorder') {
													...
													getToQuantityCheckObj.warehouseLocationId = warehouseLocationId;
													getToQuantityCheckObj.scannedQuantity = scannedQuantity;
													// getToQuantityCheckObj.drNumber = drNumber;
													getToQuantityCheckObj.transactionDetailsArray = transactionDetailsArray;
													transactionDetailsArray = inboundLib.getToQuantityCheck(getToQuantityCheckObj);
												...
												if (randomTallyScanRule == 'T' && ((backBtnVisibility == true && transactionDetailsArray.remainingQuantity == 0) ||
													...
													transactionDetailsArray.isSamePageNavigationRequired = false;
													if (btnClicked == 'Skip') {
														var binScanRequire = _checkIsInventoryItemRecevied(transactionInternalId, locationUseBinlFlag, preparedBy);
				...
				log.error({
					title: 'transactionDetailsArray',
					details: transactionDetailsArray
				});
				log.error({
					title: 'errorMessage',
		...
		function validateSerialFromIF(transactionInternalId, itemInternalId, transactionLineNo, inventoryStatus, serialName) {
			...
			var links = trecord.getLineCount({
				sublistId: 'links'
			});
			if (utility.isValueValid(links)) {
				...
				for (var itr = 0; itr < links; itr++) {
					var linkid = trecord.getSublistValue({
						sublistId: 'links',
						fieldId: 'id',
						line: itr
					});
				...
				for (var j = 0; j < searchResultsLength && IsValidSerailNumber == false; j++) {
					...
					if (searchResults[j].recordtype == 'itemfulfillment') {
						...
						for (var f = 0; f < fulfillmentItemCount && IsValidSerailNumber == false; f++) {
							var fulfillmentItem = frecord.getSublistValue({
								sublistId: 'item',
								fieldId: 'item',
								line: f
							});
							var fulfillmentLineNo = frecord.getSublistValue({
								sublistId: 'item',
								fieldId: 'orderline',
								line: f
							});
							...
							if (fulfillmentItem == itemInternalId && parseInt(transactionLineNo) == (parseInt(toFulfillmentLineNo))) {
								var serialnumbers = frecord.getSublistValue({
									sublistId: 'item',
									...
									if (lineRemQty == 0) {
										if (tallyLoopObj[key] != undefined) {
											...
											if (utility.isValueValid(tallyLoopObjectOpentaskId)) {
												...
												openTaskQty = openTaskRecord.getValue({
													fieldId: 'custrecord_wmsse_act_qty'
												});
												// drNumber = openTaskRecord.getValue({
												// 	fieldId: 'custrecord_wmsse_drnumber'
												// });
		...
		function _getSerials(orderInternalid, lineno, preparedBy) {
			...
			log.debug('_getSerials preparedBy', preparedBy);
		...
		function _getOpenTask(orderInternalid, lineno, inventoryStatusFeature, inventoryStatus, locuseBinsFlag, lotName, scannedLotArr, preparedBy) {
			var myOpentaskQuery = query.create({
				type: 'customrecord_wmsse_trn_opentask'
			});
			var tranCond = myOpentaskQuery.createCondition({
			...
			log.debug('_getOpenTask preparedBy', preparedBy);
			var userIdCondition = myOpentaskQuery.createCondition({
				fieldId: 'custrecord_wmsse_upd_user_no',
				operator: query.Operator.ANY_OF,
				values: preparedBy
		...
		function _checkIsInventoryItemRecevied(orderInternalid, locuseBinsFlag, preparedBy) {
			var myOpentaskQuery = query.create({
				type: 'customrecord_wmsse_trn_opentask'
			});
			var tranCond = myOpentaskQuery.createCondition({
			...
			log.debug('_checkIsInventoryItemRecevied preparedBy', preparedBy);
			var userIdCondition = myOpentaskQuery.createCondition({
				fieldId: 'custrecord_wmsse_upd_user_no',
				operator: query.Operator.ANY_OF,
				values: preparedBy
