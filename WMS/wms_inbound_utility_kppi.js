 ...
 * @NModuleScope public
 */
define(['N/search', 'N/runtime', 'N/record', 'N/format', './big_open', './wms_translator_kppi', './wms_utility_kppi', 'N/query', './wms_inboundUtility_kppi'],
	function (search, runtime, record, format, Big, translator, utility, query, inboundUtility) {
		function consolidatePostItemReceipt(trecord, actQuantity, linenum, itemId, transactionType, batchNo, expiryDate, whLocationId, enterBin, serialArray, opentaskSearchResults, transactionInternalId, itrValue, randomTallyScanRule, selectedRows) {
			try {
				...
				selectedItemIds = selectedRows.map(function (row) {
				...
				for (var irItr = 0; irItr < polinelength; irItr++) {
					item_id = String(trecord.getSublistValue({
						sublistId: 'item',
						fieldId: 'item',
						line: irItr
					}));
					if (selectedItemIds.indexOf(item_id) === -1) {
						trecord.setSublistValue({
							sublistId: 'item',
							fieldId: 'itemreceive',
							line: irItr,
						...
						if (itemLineNo == linenum) {
							...
							for (var otItr = 0; otItr < opentaskSearchResults.length; otItr++) {
								var openTask = opentaskSearchResults[otItr];
								if (isOpenTaskSelected(openTask, selectedRows, linenum)) {
									hasSelectedOpenTask = true;
									var actlineQuantity = openTask['custrecord_wmsse_act_qty'];
									totallineQty = Big(totallineQty).plus(actlineQuantity);
									if (utility.isValueValid(openTask['custrecord_wmsse_restock']) && (openTask['custrecord_wmsse_restock'] == true || openTask['custrecord_wmsse_restock'] == false)) {
										var restock = openTask['custrecord_wmsse_restock'];
									}
								}
							}
							if (!hasSelectedOpenTask || Number(totallineQty) <= 0) {
								log.debug({
									title: 'Skipping Unselected Item Receipt Line',
									details: {
										linenum: linenum,
										itemId: itemId,
										totalLineQty: String(totallineQty)
									}
								});
							...
							if (itemType == "serializedinventoryitem" || itemType == "serializedassemblyitem" || itemType == "lotnumberedinventoryitem" ||
								...
								if (itemType == "lotnumberedinventoryitem" || itemType == "lotnumberedassemblyitem") {
									for (var r2 = 0; r2 < opentaskSearchResults.length; r2++) {
										...
										if (isOpenTaskSelected(opentaskSearchResults[r2], selectedRows, linenum)) {
									...
									if (noBinFlag) {
										for (var r1 = 0; r1 < opentaskSearchResults.length; r1++) {
											...
											if (isOpenTaskSelected(opentaskSearchResults[r1], selectedRows, linenum)) {
												compSubRecord.insertLine({
													sublistId: 'inventoryassignment',
													line: complinelength
												});
									...
									for (var r3 = 0; r3 < opentaskSearchResults.length; r3++) {
										...
										if (isOpenTaskSelected(opentaskSearchResults[r3], selectedRows, linenum)) {
		...
		function isOpenTaskSelected(openTask, selectedRows, linenum) {
			if (!selectedRows || !selectedRows.length)
				return false;

			var openTaskLine = String(openTask['custrecord_wmsse_line_no'] || '').trim();
			var openTaskItemId = String(openTask['custrecord_wmsse_sku'] || '').trim();
			var openTaskDRNo = String(openTask['custrecord_wmsse_drnumber'] || '').trim();

			for (var i = 0; i < selectedRows.length; i++) {
				var selectedItemId = String(selectedRows[i].itemId || '').trim();
				var selectedDRNo = String(selectedRows[i].drnum || '').trim();

				if (openTaskLine === String(linenum) &&
					openTaskItemId === selectedItemId &&
					openTaskDRNo === selectedDRNo) {
					log.debug("openTaskLine: " + openTaskLine + ", String(linenum): " + String(linenum), 'true');
					log.debug("openTaskItemId: " + openTaskItemId + ", selectedItemId: " + selectedItemId, 'true');
					log.debug("openTaskDRNo: " + openTaskDRNo + ", selectedDRNo: " + selectedDRNo, 'true');
					return true;
				} else {
					log.debug("openTaskLine: " + openTaskLine + ", String(linenum): " + String(linenum), 'false');
					log.debug("openTaskItemId: " + openTaskItemId + ", selectedItemId: " + selectedItemId, 'false');
					log.debug("openTaskDRNo: " + openTaskDRNo + ", selectedDRNo: " + selectedDRNo, 'false');
				}
			}
			return false;
		}
		...
		function postItemReceipt(objPostIrValues) {
			...
			if ((idl != '' && idl != null && idl != 'null' && idl != undefined) || (systemRule != null && systemRule != '' && systemRule == 'Y')) {
				...
				opentaskObj.drNumber = objPostIrValues.drNumber;
				opentaskObj.preparedBy = objPostIrValues.preparedBy;

				var preparedBy = opentaskObj.preparedBy;
				...
				log.debug('preparedBy parameter', preparedBy);
				...
				var employeeId = empSearchResultsvalues[0].id;

				log.debug('empSearchResultsvalues', empSearchResultsvalues);
				log.debug('empSearchResultsvalues[0].id', employeeId);

				preparedBy = parseInt(employeeId); // ? employeeId : runtime.getCurrentUser().id;

				log.debug('preparedBy ID', preparedBy);
				...
				if (isTallyScanRequired == true && itemType != "serializedinventoryitem" && itemType != "serializedassemblyitem") {
					for (var obj in tallyLoopObj) {
						...
						idl = inboundUtility.updateOpenTask(opentaskObj, preparedBy);
					}
				} else {
					idl = inboundUtility.updateOpenTask(opentaskObj, preparedBy);
					if (itemType == "serializedinventoryitem" || itemType == "serializedassemblyitem") {
		...
		function getKitOTResultsforIRPosting(transactionInternalId, kititemInternalId, warehouseLocationId, objPostIRValues) {
			...
			if (utility.isValueValid(objPostIRValues)) {
				objPostItemReceiptDetails.push({
					...
					'custrecord_wmsse_drnumber': objPostIRValues.drNumber,
					'custrecord_wmsse_upd_user_no': objPostIRValues.preparedBy,
		...
		function updateorCloneOpenTask(itemReceiptId, irUpdateOpenTaskId, splitOpenTaskId) {
			...
			for (var opentaskId in splitOpenTaskId) {
				...
				var preparedBy = openTaskRec.getValue({
		...
		return {
			...
			isOpenTaskSelected: isOpenTaskSelected,
