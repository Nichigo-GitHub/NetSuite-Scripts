 ...
 * @NModuleScope Public
 */
define(['./wms_utility_kppi', './wms_workOrderUtility_kppi', './big_open', './wms_translator_kppi'],
	/**
	 * @param {search} search
	 */
						...
						if (openTaskResults.length > 0) {
							...
							if (scannedQuantityInBaseUnits > remBuildableQuantity || scannedQuantity <= 0) {
								...
								if ((itemType == 'serializedinventoryitem' || itemType == 'serializedassemblyitem') && scannedQuantity.toString().indexOf('.') != -1) {
									...
									log.debug({ title: 'transactionResults :', details: transactionResults });
								...
								log.debug({ title: 'response[isValid] :', details: response['isValid'] });
								if (response['isValid'] == true) {
									...
									log.debug({ title: 'WO Assembly Qty Validate - final pageNavigationStatus', details: response['pageNavigationStatus'] });
								}
							}
