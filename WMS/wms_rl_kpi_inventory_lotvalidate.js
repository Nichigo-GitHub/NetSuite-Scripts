 ...
 * @NModuleScope Public
 */
define(['N/search','./wms_utility','./big','./wms_translator','N/format', 'N/log'],
		/**
		 * @param {search} search
		 */
		function(search,utility,Big,translator,format,log) {
	...
	function doPost(requestBody) {
		...
		try{
			if(utility.isValueValid(requestBody)) {
				...
				log.error({title:'requestParams',details:requestParams});
				...
				if(putawayAll=='putawayAll') {
					...
					lotValidateArr["isValid"]=true;
						...
						utility.isValueValid(itemInternalId)) {
							log.error({title:'gate 1',details:'pass'});
					var objBinDetails = [];
					...
					if(!utility.isValueValid(EntLotId)) {
						log.error({title:'gate 2',details:'pass'});
						...
						lotValidateArr["isValid"]=false;
							...
							log.error({title:'gate 3',details:'pass'});
						...
						if(utility.isValueValid(itemInternalId)) {
							log.error({title:'gate 4',details:'pass'});
						...
						/* if(utility.isValueValid(fromBinInternalId)) {
							log.error({title:'gate 5',details:'pass'});
							filters.push(search.createFilter({
								name:'binnumber',
								operator:search.Operator.ANYOF,
						...
						if(utility.isValueValid(warehouseLocationId)) {
							log.error({title:'gate 6',details:'pass'});
							filters.push(search.createFilter({
								name:'location',
								operator:search.Operator.ANYOF,
						...
						if(utility.isValueValid(EntLotId)) {
							log.error({title:'gate 7',details:'pass'});
							filters.push(search.createFilter({
					...
					if(objBinDetails.length>0) {						
						log.error({title:'gate 8',details:'pass'});
						var binLocArr = [];
						...
						lotValidateArr["errorMessage"] = translator.getTranslationString('BINTRANSFER_LOTVALIDATE.INVALID_LOT');
						lotValidateArr["isValid"]=false;
						log.error({title:'gate 8',details:'failed'});
					}
				}
				else {
					lotValidateArr["errorMessage"] = translator.getTranslationString('BINTRANSFER_LOTVALIDATE.INVALID_LOT');
