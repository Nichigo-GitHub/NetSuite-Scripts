 ...
 * @NModuleScope public
 */
define(['N/search', 'N/runtime', 'N/record', 'N/config', 'N/format', './big_open', './wms_translator_kppi', 'N/url', './wms_utility_kppi', 'N/task', 'N/query'],
	function (search, runtime, record, config, format, Big, translator, url, utility, task, query) {
		...
		function _getRecevingOrderItemDetails(orderNumber, itemID, wareHouseLocationId, orderLineNo, orderInternalId, colsArr, tranType, crossSubsidiaryFeature, centralizesPurchasingandBilling, warehouseLocationName) {
			...
			if (ISMStatusFeature) {
				...
				var searchName = 'customsearch_wmsse_rcv_poitem_details_3';
				var vType = 'PurchOrd';
			}
		...
		function updateOpenTask(objOpentaskDetails, preparedBy) {
			...
			var drNumber = objOpentaskDetails['drNumber'];
			...
			if (utility.isValueValid(objOpentaskDetails['userId'])) {
				log.debug("objOpentaskDetails['userId']", objOpentaskDetails['userId']);
				openTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_upd_user_no',
					value: objOpentaskDetails['userId']
				});
			} else {
				log.debug("preparedBy inboundUtility", preparedBy);
				openTaskRecord.setValue({
					fieldId: 'custrecord_wmsse_upd_user_no',
					value: preparedBy
				});
		...
		function _getOpenPutawayTasksforIRPosting(transactionInternalId, warehouseLocationId) {
			var openPutwTasksDetails = search.load({
				id: 'customsearch_wmsse_openputaways_detail_2'
			});
		...
		function getopentaskresultsforIRPosting(poid, containerID, item_id, itemLineNo) {
			...
			if (objPostItemReceiptDetails.length > 0) {
				for (var objOPentask in objPostItemReceiptDetails) {
					...
					var drNumber = objPostItemReceiptDetails[objOPentask]['custrecord_wmsse_drnumber'];
					...
					var currRow = [linenum, itemid, actQuantity, enterBin, batchno, expiryDate, serailNum, opentaskId, wmsLocation, vInvStatus_select, drNumber];
		...
		function noCodeSolForReceiving(orderInternalId, orderLineNo, itemReceipt, transactionType, openTaskIdarray, locationUseBinlFlag, itemType, preparedBy) {
			var itemreceiptArr = [];
			var openTaskArr = [];
			...
			if (preparedBy)
				impactedRecords['custrecord_wmsse_upd_user_no'] = preparedBy;
		...
		function buildOpenTaskParametes(requestParams, manullayPostItemReceipt, binName, binInternalId, shipmentLineNo,
			...
			objOpentaskDetails.drNumber = drNumber;
		...
		function postItemReceiptISM(shipmentInternalId, warehouseLocationId, openTaskDtls, openTaskQtyDetails, binId, randomTallyScanRule) {
			...
			for (var index = 0; index < openTaskDtls.length; index++) {
				...
				shipmentListObj.drNumber = openTaskDtls[index].custrecord_wmsse_drnumber;
				if (serialSearchResults && serialSearchResults.length > 0) {
					this.closeSerialEntryRecordsForTransaction(serialSearchResults);
				}
			}
		...
		function updateNSrefno(openTaskID, binTransferRec, actualQty, drNumber) {
		...
		function _createOrUpdateOpentask(openTaskId, quantity, objOpentaskDetails, warehouseLocationId, lotName, drNumber, preparedBy) {
			log.debug("preparedBy createOrUpdateOpentask wms_inboundUtility_kppi", preparedBy);
			if (utility.isValueValid(openTaskId)) {
				updateOpentaskFields(openTaskId, quantity, '', lotName, drNumber);
			} else {
				openTaskId = updateOpenTask(objOpentaskDetails, preparedBy);
			}
			return openTaskId;
		}

		function updateOpentaskFields(openTaskId, quantity, serialNo, lotName, drNumber) {
