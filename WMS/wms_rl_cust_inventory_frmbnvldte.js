 ...
 * @NModuleScope Public
 */
define(['N/search', './wms_utility', './big', './wms_translator', './wms_tallyScan_utility'],
	/**
	 * @param {search} search
	 */
	function (search, utility, Big, translator, tallyScanUtility) {
					...
					if (utility.isValueValid(fromBinName) && utility.isValueValid(warehouseLocationId) &&
						...
						if (utility.isValueValid(fromBinName)) {
							...
							if (processType == 'replen') {
								...
								log.debug({ title: 'binSearchResultsvalues', details: binSearchResultsvalues });
								if (utility.isValueValid(binSearchResultsvalues)) {
									binInternalId = binSearchResultsvalues[0].id;
									fromBinNumber = binSearchResultsvalues[0].getValue({ name: 'binnumber' });

								}
								log.debug({ title: 'binInternalId', details: binInternalId });
								log.debug({ title: 'preferedBinName', details: preferedBinName });
								log.debug({ title: 'fromBinName', details: fromBinName });
								...
								if (processType == 'putAway') {
									...
									if (binSearchResultsvalues != null && binSearchResultsvalues != '' && binSearchResultsvalues.length > 0) {
										if (stageLocArr.indexOf(binSearchResultsvalues[0].getValue({ name: 'custrecord_wmsse_bin_loc_type' })) == -1) {
											fromBinValidate.errorMessage = translator.getTranslationString('BINPUTW_FROM_BINVALIDATE.NOTSTAGE');
											fromBinValidate.isValid = false;

										}
										else if (binSearchResultsvalues[0].getValue({ name: 'custrecord_wmsse_bin_stg_direction' }) != 1) {
									...
									log.debug('Info', 'Inventory Transfer');
									binSearchResults = search.load({
										id: 'customsearch3809'
									});
									binSearchFilters = binSearchResults.filters;
									...
									if (utility.isValueValid(binSearchResults)) {
										...
										fromBinNumber = binSearchResults[0].getValue({ name: 'custrecord_bin_code' });
									}
									...
									if (!(utility.isValueValid(binInternalId))) {
										...
										else if (binSearchResults[0].getText({ name: 'custrecord_wmsse_bin_loc_type' }) == 'WIP' &&
											binSearchResults[0].getValue({ name: 'custrecord_wms_mfg_picking' }) == false) {
							...
							if (!(utility.isValueValid(binInternalId))) {
								...
								if (itemType == "inventoryitem" || itemType == "assemblyitem" || itemType == "serializedinventoryitem" || itemType == "serializedassemblyitem") {
									if (inventoryStatusFeature) {
										...
										else if (itemType == translator.getTranslationString("ITEMTYPE_SERIAL") || itemType ==
											translator.getTranslationString("ITEMTYPE_SERIAL_ASSEMBLY")) {
											searchName = 'customsearch_wmsse_invtbalance_serialsrh';

										}

										objBinDetails = getItemWiseInventoryDetailswithInvStatusEnable(searchName, itemInternalId, binInternalId, warehouseLocationId,
											makeInvtAvailableFlag);
									}
									else {
									...
									if (inventoryStatusFeature) {
										...
										objBinDetails = getItemWiseLotsDetails(itemInternalId, warehouseLocationId, binInternalId);
									}
								}
								if (objBinDetails.length == 0) {
									...
									if (objBinDetails.length > 0) {
										...
										if (availableQty == 0 || availableQty < 0) {
											log.debug({ title: 'availqty', details: availableQty });
											fromBinValidate.errorMessage = translator.getTranslationString('BINTRANSFER_FROMBINVALIDATE.INVALID_INPUT');
											fromBinValidate.isValid = false;
										}
									}
								...
								if (objBinDetails.length > 0 && availableQty > 0) {
									...
									if (fromBinValidate.isTallyScanRequired) {
										if (utility.isValueValid(itemAliasUnit)) {
											...
											barcodeQuantity = [{ 'value': '1', 'unit': fromBinValidate.barcodeQuantity[0].unit }];
										}
									}
									fromBinValidate.barcodeQuantity = barcodeQuantity;
									log.debug('return fromBinValidate', fromBinValidate);
								}
							}
						}
						else {
		...
		function getItemWiseInventoryDetails(ItemInternalId, strLocation, vBinId) {
			var searchObj = search.load({ id: 'customsearch_wmsse_itemwise_inventory' });
		...
		function getItemWiseLotsDetails(ItemInternalId, strLocation, vBinId) {
