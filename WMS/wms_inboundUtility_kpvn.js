 ...
 * @NModuleScope public
 */
define(['N/search', 'N/runtime', 'N/record', 'N/config', 'N/format', './big_open', './wms_translator_kpvn', 'N/url', './wms_utility_kpvn', 'N/task', 'N/query'],
	function (search, runtime, record, config, format, Big, translator, url, utility, task, query) {
		...
		function _getRecevingOrderItemDetails(orderNumber, itemID, wareHouseLocationId, orderLineNo, orderInternalId, colsArr, tranType, crossSubsidiaryFeature, centralizesPurchasingandBilling, warehouseLocationName) {
			...
			if (ISMStatusFeature) {
				...
				var searchName = 'customsearch_wmsse_rcv_poitem_details_2';
				var vType = 'PurchOrd';
			}
				...
				objOpentaskDetailsSearch.run().each(function (result) {
					if (utility.isValueValid(result)) {
						opentaskArr[result.getValue({
							name: 'custrecord_wmsse_line_no',
							summary: 'GROUP'
						})] = result.getValue({ name: 'custrecord_wmsse_act_qty', summary: 'SUM' });
					}
					return true;
				});
			}
		...
		function updateOpenTask(objOpentaskDetails) {
			...
			log.debug({ title: 'objOpentaskDetails', details: objOpentaskDetails });
			log.debug({ title: 'expectedQty', details: expectedQty });
			var openTaskRecord = record.create({
				type: 'customrecord_wmsse_trn_opentask'

			});
			if (utility.isValueValid(restock) && restock == false) {
				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_restock', value: false });
			} else if (utility.isValueValid(restock) && restock == true) {
				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_restock', value: true });
			}
			...
			if (utility.isValueValid(shipmentReceiptId)) {
				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_expe_qty', value: Number(Big(expectedQty).toFixed(5)) });
			}
			else {
			...
			if (utility.isValueValid(enterBin)) {
				if (utility.isValueValid(randomTallyscan) && randomTallyscan == 'T') {
					openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_reccommendedbin', value: enterBin });

					if (utility.isValueValid(recomendedBinSequenceNo)) {
						openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_recomendedbinsequence', value: recomendedBinSequenceNo });
					}
				} else {
			...
			if (utility.isValueValid(randomTallyscan) && randomTallyscan == 'T') {
				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_notes', value: 'randomTallyscan' });
			}
			...
			openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_parent_sku_no', value: utility.isValueValid(objOpentaskDetails.parentItem) ? objOpentaskDetails.parentItem : FetchedItemId });
			...
			if (utility.isValueValid(objOpentaskDetails['userId'])) {
				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_upd_user_no', value: objOpentaskDetails['userId'] });
			} else {
			...
			if (utility.isValueValid(shipmentReceiptId)) {
				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_rec_inb_shipment', value: shipmentReceiptId });
			}
			if (utility.isValueValid(shipmentId)) {
				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_inbshipment', value: shipmentId });
			}
			...
			if (utility.isValueValid(binIsCart) && utility.isValueValid(binLocType) && binIsCart == true && binLocType == 'Stage') {
				...
				if (utility.isValueValid(objRecommendedBinData)) {
					...
					if (utility.isValueValid(vInvStatus_select)) {
						openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_inventorystatusto', value: vInvStatus_select });
					}

					if (utility.isValueValid(objRecommendedBinData.recomendedBinId)) {
						openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_reccommendedbin', value: objRecommendedBinData.recomendedBinId });
					}
					if (utility.isValueValid(objRecommendedBinData.recomendedBinSequenceNo)) {
						openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_recomendedbinsequence', value: objRecommendedBinData.recomendedBinSequenceNo });
					}
					if (utility.isValueValid(objRecommendedBinData.puStratagieId)) {
						openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_put_strategy', value: objRecommendedBinData.puStratagieId });
					}
				}
			}
			if (utility.isValueValid(objOpentaskDetails.isKitComponent)) {
				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_kitflag', value: objOpentaskDetails.isKitComponent });
			}
		...
		function _getOrderStatus(orderInternalId, getTranId, tranType, warehouseLocationName, centralizesPurchasingandBilling) {
			log.debug({ title: '_getOrderStatus', details: '_getOrderStatus' });
			var vType = 'PurchOrd';
			if (tranType == 'transferorder') {
				vType = 'TrnfrOrd';
			}
			...
			log.debug({ title: 'orderSearchResults', details: orderSearchResults });
		...
		function checkCountforRemaining(shipmentListDetails, shipmentNum, whLocation) {
			...
			if (utility.isValueValid(shipmentListDetails) && shipmentListDetails.length > 0) {
				...
				log.debug({ title: 'receivedOpentaskDetails', details: receivedOpentaskDetails });
				for (var i in shipmentListDetails) {
					...
					if (JSON.stringify(receivedOpentaskDetails) != '{}') {
						...
						log.debug({ title: 'lineItemReceivedOpentaskQty :', details: lineItemReceivedOpentaskQty });
			...
			log.debug({ title: 'vCount :', details: vCount });
			return vCount;

		}
		function getInbShipmentReceivedOpenTaskDetails(shipmentId, whLocation, shipmentLineNo) {

			log.debug({ title: 'getInbShipmentReceivedOpenTaskDetails shipmentId', details: shipmentId });
			log.debug({ title: 'getInbShipmentReceivedOpenTaskDetails whLocation', details: whLocation });
			var opentaskArr = {};
			var filterStrat = [];
			if (utility.isValueValid(shipmentId)) {
				if (utility.isValueValid(whLocation)) {
				...
				var objOpentaskDetailsSearch = search.load({ id: 'customsearch_wms_inbship_rcvdopentaskdtl' });
				var savedFilter = objOpentaskDetailsSearch.filters;
				objOpentaskDetailsSearch.filters = savedFilter.concat(filterStrat);
				var objOPentaskDetails = utility.getSearchResultInJSON(objOpentaskDetailsSearch);

				log.debug({ title: 'getInbShipmentReceivedOpenTaskDetails objOPentaskDetails :', details: objOPentaskDetails });
			...
			log.debug({ title: 'receivedOpentaskArr', details: opentaskArr });
			return opentaskArr;
		...
		function _checkIslinePartiallyReceivedInISM(shipmentId, whLocation, getOrderLineNo) {
			...
			if (JSON.stringify(openPutawayResults) != '{}') {
				...
				log.debug({ title: 'OpentaskQty :', details: lineItemReceivedOpentaskQty });
		...
		function getItemShipments(itemArray, warehouseLocationId) {
			var itemShipmentSearch = search.load({ id: 'customsearch_wms_item_based_is_search' });
		...
		function getItemShipmentList(itemInternalId, warehouseLocationId, vendorId) {
			var itemShipmentListSearch = search.load({ id: 'customsearch_wms_inbound_shipment_valid' });
		...
		function _getReceiveAllOptionValue(whLocation, transactionType) {
			...
			if (recevAllSystemRule == 'Y') {
				...
				if (utility.isValueValid(whLocation)) {
					...
					log.debug({ title: 'locationLookUp', details: locationLookUp });

					if (locationLookUp.usesbins != undefined) {
						locationUseBinsFlag = locationLookUp.usesbins;
					}
				}
				log.debug({ title: 'locationUseBinsFlag', details: locationUseBinsFlag });
				if (locationUseBinsFlag == true) {
					...
					log.debug({ title: 'stageRecvngSystemRule', details: stageRecvngSystemRule });
		...
		function receiveISM(shipmentId, warehouseLocationId, binInternalId, shipmentLineNo, quantity, poInternalId, itemType, invtStatus, isTallyScanRequired, tallyLoopObj, selectedConversionRateForTallyScan, transactionUomConversionRate) {
			...
			log.debug({ title: 'receiveISM', details: 'receiveISM' });
		...
		function setLineDetailsForShipmentRecord(inventoryStatusId, objRecord, inboundShipmentItemcount, shipmentId, warehouseLocationId, binInternalId, shipmentLineNo, quantity, poInternalId, itemType, isTallyScanRequired, tallyLoopObj, selectedConversionRateForTallyScan, transactionUomConversionRate) {
			log.debug({ title: 'inventoryStatusId', details: inventoryStatusId });
							...
							if (isTallyScanRequired == true && (itemType == 'lotnumberedinventoryitem' || itemType == 'lotnumberedassemblyitem')) {
								for (var obj in tallyLoopObj) {
									...
									if (utility.isValueValid(tallyObj.lotExpiryDate)) {
										var lotExpiryDate = format.parse({ value: tallyObj.lotExpiryDate, type: format.Type.DATE });
										compSubRecord.setSublistValue({
											sublistId: 'inventoryassignment',
		...
		function _iSM_receiveAll(shipmentId, warehouseLocationId, binInternalId, shipmentLineNo, manuallyPostIrSystemRule, invtStatusId) {
			...
			log.debug({ title: 'shipmentLineNo', details: shipmentLineNo });
			...
			for (var lineNumber = 0; lineNumber < inboundShipmentItemcount; lineNumber++) {
				...
				if (shipmentLineNo == lineNo || shipmentLineNo == 'receiveAllItems') {
					...
					log.debug({ title: 'itemISMReceivedQty', details: itemISMReceivedQty });
					...
					for (var invDetailLineCount = 0; invDetailLineCount < compinvlinelength; invDetailLineCount++) {
						...
						log.debug({ title: 'serial Id', details: id });
					}
		...
		function _createLotISMInfoInSerialEntry(paramsLotISMObj, manuallyPostIrSystemRule) {
			...
			customrecord.setValue({ fieldId: 'name', value: shipmentInternalNumber });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_ordno', value: purchaseorderInternalId });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_ordline', value: shipmentLineNumber });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_item', value: itemInternalId });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_qty', value: scannedQuantity });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_no', value: LotName });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_location', value: warehouseLocationId });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_inb_shipment', value: shipmentInternalNumber });
			if (utility.isValueValid(manuallyPostIrSystemRule) && manuallyPostIrSystemRule == 'N') {
				customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_status', value: true });
			}
			else {
			...
			if (utility.isValueValid(inventoryStatus)) {
				customrecord.setValue({ fieldId: 'custrecord_serial_inventorystatus', value: inventoryStatus });
			}

			if (utility.isValueValid(lotExpiryDate)) {
				customrecord.setValue({ fieldId: 'custrecord_wmsse_lot_expirydate', value: lotExpiryDate });
			}
			if (utility.isValueValid(binInternalId)) {
		...
		function setLineDetailsForShipmentRecordIRPosting(objRecord, inboundShipmentItemcount, shipmentListObj, openTaskDtls) {
			...
			for (var lineNumber = 0; lineNumber < inboundShipmentItemcount; lineNumber++) {
				...
				if (shipmentLineNo == lineNo) {
						...
						if (itemType == translator.getTranslationString("ITEMTYPE_SERIAL") ||
							...
							if (serialSearchResults.length > 0) {
								...
								for (var serialLotArr = 0; serialLotArr < serialSearchResults.length; serialLotArr++) {
									...
									if (utility.isValueValid(lotExpiryDate)) {
										compSubRecord.setSublistValue({
											sublistId: 'inventoryassignment', fieldId: 'expirationdate',
					...
					objRecord.setSublistValue({
						sublistId: 'receiveitems', fieldId: 'receiveitem',
				...
				for (var d = 0; d < TOLineDetails.length; d++) {
					...
					if (itemfulfillmentId != null && itemfulfillmentId != 'null' && itemfulfillmentId != undefined &&
						...
						for (var f = 0; f < fulfillmentItemCount; f++) {
							var fitem = frecord.getSublistValue({ sublistId: 'item', fieldId: 'item', line: f });
							var fline = frecord.getSublistValue({ sublistId: 'item', fieldId: 'orderline', line: f });
							var inventorydetail = frecord.getSublistValue({ sublistId: 'item', fieldId: 'inventorydetail', line: f });
								...
								if (itemType == "inventoryitem" || itemType == "assemblyitem") {
									...
									for (var cnt = 0; cnt < ordlinelength; cnt++) {
										...
										if (inventoryStatusFeature) {
											...
											log.debug({ title: 'invtStatus', details: invtStatus });
									...
									for (var cnt = 0; cnt < ordlinelength; cnt++) {
										...
										var fulfilledLotName = compSubRecord.getSublistText({ sublistId: 'inventoryassignment', fieldId: 'issueinventorynumber', line: cnt });
										if (inventoryStatusFeature) {
											fromStatusInternalId = compSubRecord.getSublistValue({ sublistId: 'inventoryassignment', fieldId: 'inventorystatus', line: cnt });
		...
		function createRecordInWMSSerialEntryCustomRecord(paramsSerialObj) {
			...
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_ordno', value: transactionInternalId });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_ordline', value: transactionLineNo });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_item', value: itemInternalId });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_qty', value: quantity });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_no', value: serialName });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_status', value: serialStatus });
			customrecord.setValue({ fieldId: 'custrecord_wmsse_ser_tasktype', value: taskType });
			if (utility.isValueValid(inventoryStatus)) {
				customrecord.setValue({ fieldId: 'custrecord_serial_inventorystatus', value: inventoryStatus });
			}
			if (utility.isValueValid(shipmentId)) {
			...
			var serialEntryRecordId = customrecord.save({ enableSourcing: false, ignoreMandatoryFields: true });
			return serialEntryRecordId;
		}
		...
		function getListObjectInternalId(listObjectName, listId) {
			var BinlocationSearch = search.create({ type: listId, columns: [{ name: 'name' }] });
			var BinlocationTypes = BinlocationSearch.run().getRange({ start: 0, end: 1000 });
			var listObjectId = '';
		...
		function getStageBinDetails(objItemDetails, processType, invstatus, inventoryStatusFeature, stageDir) {
			...
			log.debug({ title: 'fromBinInternalId1', details: fromBinInternalId });

			var objBinDetailsSearch = search.load({ id: 'customsearch_stage_bindetails' });
			...
			log.debug({ title: 'outStageDirectionId', details: outStageDirectionId });
			log.debug({ title: 'stageLocId', details: stageLocId });
			log.debug({ title: 'inStageDirectionId', details: inStageDirectionId });
				...
				if (objBinStatusResults.length > 0) {
					for (var stsItr in objBinStatusResults) {
						...
						if (fromBinInternalId != binInternalId) {
							...
							var currRow = {
								'binName': binName, 'binInternald': binInternalId, 'quantity': binQty, 'quantityWithUOM': binQtyAvailWithUOM,
		...
		function _callSchedulerToReceiveISM(shipmentId, warehouseLocationId, binInternalId, receiveAllStatusId) {
			log.debug({ title: 'Map Reduce - shipmentId :', details: shipmentId });
		...
		function getRecommendedBinForCartMoves(itemInputObj) {
			...
			if (utility.isValueValid(recBinDtlsObj.recomendedBinId)) {
				...
				var binRec = search.lookupFields({ type: 'Bin', id: recBinDtlsObj.recomendedBinId, columns: fields });
		...
		function _closeSerialInSerialEntry(serialEntryID, serial) {
			...
			serialRec.setValue({ fieldId: 'id', value: serialEntryID });
			serialRec.setValue({ fieldId: 'name', value: serial });
		...
		function updateOpentaskFields(openTaskId, quantity, serialNo, lotName) {
			...
			if (utility.isValueValid(quantity)) {
				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_act_qty', value: quantity });
				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_expe_qty', value: quantity });
			}
			if (utility.isValueValid(serialNo)) {
				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_serial_no', value: serialNo.toString() });
			}
			if (utility.isValueValid(lotName)) {

				openTaskRecord.setValue({ fieldId: 'custrecord_wmsse_batch_num', value: lotName });
			}
