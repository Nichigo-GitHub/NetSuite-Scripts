		...
		function doPost(requestBody) {
			...
			try {
				...
				if (utility.isValueValid(requestBody)) {
					...
					// Log requestBody for debugging
					log.debug({
						title: 'requestBody',
						details: requestBody
					});

					// Initialize itemReceiptId
					var itemReceiptId = '';
					...
					if (utility.isValueValid(warehouseLocationId) && utility.isValueValid(transactionInternalId) && utility.isValueValid(transactionName) &&
						...
						// Check if manual item receipt posting is allowed for the warehouse location and transaction type
						...
						if (systemRule == 'Y') {
							...
							if (PORec.length > 0) {
								...
								// Log targetSubsidiary for debugging
								log.debug({
									title: 'targetSubsidiary',
									details: targetSubsidiary
								});

								// Check if poToLocationID is valid
								if (poToLocationID == null || poToLocationID == '') {
									poToLocationID = warehouseLocationId;
								}

								// Check if the order status allows item receipt posting
								if (poStatus == 'pendingReceipt' || poStatus == 'partiallyReceived' || poStatus == 'pendingRefundPartReceived' ||
									...
									// Initialize isMapReduceScriptInvoked and useitemcostflag
									var isMapReduceScriptInvoked = 'F';
									var useitemcostflag = '';

									// Check if item cost flag is required for transfer orders
									...
									// Process based on transfer order and useitemcostflag
									if (transactionType == "transferorder" && useitemcostflag == true) {
										// Process based on the number of open tasks for item receipt posting
										...
										if (opentaskSearchResults.length > 50) {
											// Submit Map/Reduce script for item receipt posting
										...
										if (opentaskSearchResults.length > 0 && isMapReduceScriptInvoked == 'F') {
											// Consolidate and post item receipt for transfer orders
										...
										if (opentaskSearchResults.length > 0 && isMapReduceScriptInvoked == 'F') {
											...
											if (trecord != null && trecord != '') {
												...
												// Load the saved record to modify it
												var loadedRecord = record.load({
													type: record.Type.ITEM_RECEIPT, // replace with the actual type if different
													id: itemReceiptId
												});

												// Set a field value
												loadedRecord.setValue({
													fieldId: 'tranid', // replace with the actual field ID
													value: rrnumber // replace with the value you want to set
												});

												// Save the record again after modification
												loadedRecord.save({
													enableSourcing: true,
