 ...
 * @NModuleScope Public
 */
define(["N/search", "N/config", "./wms_utility_kppi", "./big_open", "./wms_translator_kppi", "./wms_inboundUtility_kppi", "./wms_inbound_utility_kppi", "N/record", "N/runtime", "N/task"],
  /**
   * @param {search} search
   */
  function (search, config, utility, Big, translator, inboundUtility, inboundLib, record, runtime, task) {
    /**
      ...
      try {
        if (utility.isValueValid(requestBody)) {
          ...
          if (utility.isValueValid(whLocation) && utility.isValueValid(orderInternalId) &&
            utility.isValueValid(tranid) && utility.isValueValid(fetchedItemId) && utility.isValueValid(orderLineNo)
          ) {
            var itemSearchResults = this.getItemCustomDetails(fetchedItemId, whLocation);
            debugString = debugString + ",itemSearchResults :" + itemSearchResults;
            ...
            if (itemSearchResults.length == 0) {
              ...
              if (!utility.isValueValid(conversionRate)) {
                if (utility.isValueValid(itemSearchResults[0]["unitstype"]))
                  conversionRate = utility.getStockCoversionRate(itemSearchResults[0]["unitstype"], uom, "1");
              }
              ...
              if (utility.isValueValid(enterBin)) {
                ...
                var systemRule = utility.getSystemRuleValueWithProcessType("Stage received items before putting away?", whLocation, transactionType);
                ...
                if ((itemType != "noninventoryitem" && itemType != "otherchargeitem" &&
                    ...
                    itemType != "giftcertificateitem") || randomTallyScanRule == "T") {
                  if (systemRule == "N") {
                    if (preferBin != enterBin) {
                      ...
                      if (stageLocArr.length > 0) {
                        ...
                        if (utility.isValueValid(binLookUp) && utility.isValueValid(binLookUp.custrecord_wms_iscart) &&
                    ...
                    if (blnMixItem == false || blnMixLot == false) {
                      if (enterBin != preferBin && binInternalId != "" && binInternalId != null) {
                        if (!blnMixItem) {
                          var objInvDetails = getItemInventoryDetails(fetchedItemId, whLocation, binInternalId);

                          if (objInvDetails != null && objInvDetails != "") {
                            if (objInvDetails.length > 0) {
                              binValidateDetails.errorMessage = translator.getTranslationString("PO_QUANTITYVALIDATE.MIXITEMS_FALSE");
                              binValidateDetails.isValid = false;
                        ...
                        if (!blnMixLot && (itemType == translator.getTranslationString("ITEMTYPE_LOT") ||
                          ...
                          if (objInvDetails != null && objInvDetails != "") {
                            if (objInvDetails.length > 0) {
                              binValidateDetails.errorMessage = translator.getTranslationString("PO_QUANTITYVALIDATE.MIXLOTS_FALSE");
                              binValidateDetails.isValid = false;
                              boolMixItemRulesStaisfied = false;
                              tempflag = false;
                            }
                    ...
                    debugString = debugString + ",binSearchResults :" + binSearchResults;
                    if (binSearchResults != null && binSearchResults != "") {
                      if (stageLocArr.indexOf(binSearchResults[0].getValue({name: "custrecord_wmsse_bin_loc_type"})) == -1) {
                        binValidateDetails.errorMessage = translator.getTranslationString("PO_QUANTITYVALIDATE.NOT_STAGEBIN");
                        binValidateDetails.isValid = false;
                        tempflag = false;
                      } else if (
                        binSearchResults[0].getValue({name: "custrecord_wmsse_bin_stg_direction"}) != 1) {
                        ...
                        binInternalId = binSearchResults[0].id;
                        binLocType = "Stage";
                        if (utility.isValueValid(binSearchResults[0].getValue({name: "custrecord_wms_iscart"})) &&
                          binSearchResults[0].getValue({name: "custrecord_wms_iscart"}) == true
                        ) {
                    ...
                    if (binSearchResults == "" || binSearchResults == null) {
                      binValidateDetails.errorMessage = translator.getTranslationString("PO_QUANTITYVALIDATE.INVALID_STAGEBIN");
                      binValidateDetails.isValid = false;
                      tempflag = false;
                    }
                  }
                ...
                if (tempflag) {
                  if (!utility.isValueValid(locUseBinsFlag)) {
                    if (utility.isValueValid(inforeceivedQuantity) && utility.isValueValid(inforemainingQuantity)) {
                      if (utility.isValueValid(uom)) {
                        ...
                        inforeceivedQuantity = parseFloat(inforeceivedQuantity.toString().substring(0, index == -1 ? 1 : index)) + parseFloat(enterQty);
                        index = inforemainingQuantity.indexOf(" ");
                        inforemainingQuantity =parseFloat(inforemainingQuantity.toString().substring(0, index == -1 ? 1 : index)) - parseFloat(enterQty);

                        inforeceivedQuantity = inforeceivedQuantity == 0 ? "0" : inforeceivedQuantity + " " + uom;
                        inforemainingQuantity = inforemainingQuantity == 0 ? "0" : inforemainingQuantity + " " + uom;
                      } else {
                        inforeceivedQuantity = parseFloat(inforeceivedQuantity) + parseFloat(enterQty);
                        inforemainingQuantity = parseFloat(inforemainingQuantity) - parseFloat(enterQty);
                      }
                    }
                  }

                  binValidateDetails.inforeceivedQuantity = inforeceivedQuantity;
                  binValidateDetails.inforemainingQuantity = inforemainingQuantity;
                  ...
                  if (systemRuleValue == "Y") {
                    var orderItemLineDetails = inboundUtility.getRecevingOrderItemDetails(tranid, fetchedItemId, whLocation, orderLineNo,
                        null, null, transactionType, crossSubsidiaryFeature, centralizesPurchasingandBilling, warehouseLocationName);
                    ...
                    if (orderItemLineDetails.length > 0) {
                      ...
                      var openPutAwayDetails = inboundUtility.getOpentaskOpenPutwayDetails( orderId, whLocation, fetchedItemId, orderLineNo);
                      if (JSON.stringify(openPutAwayDetails) !== "{}") {
                        ...
                        if (utility.isValueValid(openTaskRcvQty)) {
                          ...
                          var orderLineReceivedQty = orderLineDetailsRec.totalReceivedQty;
                          if (transactionType == "returnauthorization")
                            orderLineRemainingQty = orderLineDetailsRec.rmaRemainingQty;
                          else
                            orderLineRemainingQty = orderLineDetailsRec.poRemainingQty;
                          orderLineDetailsRec.totalReceivedQty = Number(Big(orderLineReceivedQty).plus(openTaskRcvQty));
                          orderLineDetailsRec.poRemainingQty = Number(Big(orderLineRemainingQty).plus(openTaskRcvQty));
                        }
                      }
                      var orderRemQty = 0;
                      if (transactionType == "returnauthorization")
                    ...
                    if (!tempflag) {
                      binValidateDetails.errorMessage = translator.getTranslationString("PO_QUANTITYVALIDATE.TRANSACTION_COMPLETED");
                      binValidateDetails.isValid = false;
                    }
                  }

                  debugString = debugString + ",binInternalId :" + binInternalId;
                  if (itemType == "noninventoryitem" || itemType == "otherchargeitem" || itemType == "serviceitem" ||
                    itemType == "downloaditem" || itemType == "giftcertificateitem"
                  ) {
                  ...
                  if (isTallyScanRequired == true && randomTallyScanRule == "T") {
                    ...
                    var trecord = null;
                    var opentaskSearchResults = inboundUtility.getOTResultsforIRPosting(orderInternalId, "", "", "", whLocation, randomTallyScanRule);
                    ...
                    if (opentaskSearchResults.length > 0) {
                      if (systemRuleValue == "N") {
                        ...
                        var preparedBy = "";
                        for (
                          var otItr = 0; otItr < opentaskSearchResults.length; otItr++
                        ) {
                          ...
                          if (prossedLinesArr.indexOf(linenum) == -1) {
                            ...
                            drNumber = opentaskSearchResults[otItr].custrecord_wmsse_drnumber;
                            preparedBy = opentaskSearchResults[otItr].custrecord_wmsse_upd_user_no;

                            inboundLib.consolidatePostItemReceipt(trecord, actQuantity, linenum, itemId, transactionType, batchNo, expiryDate,
                              whLocation, binInternalId, serialArray, opentaskSearchResults, orderInternalId, otItr, randomTallyScanRule, drNumber, preparedBy);
                            //itemIterator++;
                          }
                        }
                      ...
                      if (itemReceipt != "" || systemRuleValue == "Y") {
                        if (runtime.getCurrentScript().getRemainingUsage() > scriptUsageRequiredToUpdateOpenTask(opentaskSearchResults.length) &&
                          opentaskSearchResults.length <= 100
                        ) {
                          for (var j = 0; j < opentaskSearchResults.length && runtime.getCurrentScript().getRemainingUsage() >= 20; j++) {
                          ...
                          var result = triggerMapreduce(whLocation, itemReceipt, binInternalId, orderInternalId);
                          if (JSON.stringify(result) != "{}") {
                            if (result.returnStatus) {
                              if (result.returnStatus == "MapReduceTriggered" && systemRuleValue == "Y") {
                    ...
                    postIrObj.drNumber = drNumber;
                    postIrObj.preparedBy = preparedBy;
                    var barcodeDataObj = {};
                    ...
                    if (isTallyScanRequired == true) {
                      ...
                      for (var obj in tallyLoopObj) {
                        ...
                        totalQty = parseFloat(totalQty) + parseFloat(tallyObj.quantity);
                      }
                      if (utility.isValueValid(selectedConversionRateForTallyScan)) {
                        totalQty = Number(Big(totalQty).mul(selectedConversionRateForTallyScan));
                      ...
                      postIrObj.selectedConversionRate = selectedConversionRateForTallyScan;
                    }
                  ...
                  if (binValidateDetails.mapReduceTriggered != "Y") {
                    orderLineDetails = inboundUtility.getRecevingOrderItemDetails(tranid, null, whLocation, null, null, null, transactionType,
                      crossSubsidiaryFeature, centralizesPurchasingandBilling, warehouseLocationName);
                  }
                  if (orderLineDetails != null && orderLineDetails.length > 0) {
                    var orderId = orderLineDetails[0]["internalid"];
                    var openPutAwayDetails = inboundUtility.getOpentaskOpenPutwayDetails(orderId, whLocation);
                    var vCount = 0;
                    ...
                    for (var cnt in orderLineDetails) {
                      ...
                      if (JSON.stringify(openPutAwayDetails) !== "{}") {
                        ...
                        if (utility.isValueValid(itemOpentaskRcvQty)) {
                          ...
                          orderLineArr.totalReceivedQty = Number(Big(lineReceivedQty).plus(itemOpentaskRcvQty));
                          orderLineArr.poRemainingQty = Number(Big(lineRemainingQty).minus(itemOpentaskRcvQty));
                          orderLineArr.rmaRemainingQty = Number(Big(lineRemainingQty).minus(itemOpentaskRcvQty));
                        }
                      }
    ...
    function triggerMapreduce(whLocationId, itemreceiptId, binInternalId, orderID) {
      ...
      if (taskStatus.status != "FAILED") {
        utility.updateScheduleScriptStatus("randomTallyscanBinUpdate", runtime.getCurrentUser().id, "Submitted", orderID, "", "", "");
